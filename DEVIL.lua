...
file:close()
sleep(1)
os.execute(' screen -S DEVIL ./DEVIL')
end
...
DEVIL_SUDO = sudos.KpOwner   -------bot owner
DEVIL_TOKEN = sudos.Token -----DEVIL_TOKEN
bot_id = sudos.Bot_ID ---id bot
DEVIL = tonumber(_redis.Bot_ID)

function Run()
print('\27[93m>Developer:\27[39m'..' '..'@DEVIL_ch')
end
------------------------boT ID   BY DEVIL ----------------------
print(string.sub(_redis.Bot_ID,1,0))
local bot_id = redis:get(DEVIL.."Bot:KpBotAccount") or tonumber(_redis.Bot_ID)
...
redis:sadd(DEVIL.."Bot:KpSudos", user)
end
redis:setex(DEVIL.."bot:reload", 7230, true)
redis:set(DEVIL.."Bot:KpOwnerBot", config.KpOwner or 0)
redis:set(DEVIL.."Bot:Run", config.Run or 0)
local Api = config.Token:match("(%d+)")
local RD = RNM or 0
if Api then
redis:set(DEVIL.."Bot:Api_ID", Api)
end
function AuthoritiesEn()
local hash = "Bot:KpSudos"
local list = redis:smembers(DEVIL..hash)
local OwnerKP_ = redis:get(DEVIL.."Bot:KpOwnerBot")
local text = "List of Authorities :\n"
local user_info_ = redis:get(DEVIL.."user:Name" .. OwnerKP_)
...
local user_info = redis:get(DEVIL.."user:Name" .. v)
if user_info then
...
local list2 = redis:smembers(DEVIL..hash2)
if #list2 ~= 0 then
text = text .. [[
...
redis:set(DEVIL.."AuthoritiesEn", text)
end
...
redis:set(DEVIL.."AuthoritiesFa", text)
end
AuthoritiesEn()
AuthoritiesFa()
end
...
redis:del(DEVIL.."Bot:KpSudos")
redis:del(DEVIL.."Bot:KpOwnerBot")
redis:del(DEVIL.."Bot:Token")
redis:del(DEVIL.."Bot:Channel")
setdata()
end
...
usr = string.gsub(usr, "%s+$", "")
usr = string.gsub(usr, "[\n\r]+", " ")
redis:set(DEVIL.."Bot:ServerUser", usr)----------ServerUser
redis:del(DEVIL.."MatchApi")
redis:del(DEVIL.."Set_Our_ID")
redis:del(DEVIL.."Open:Chats")
local KPdata = redis:get(DEVIL.."Botid" .. bot_id) or "\n"
local BotKPdata = redis:get(DEVIL.."KpOwnerBot" .. config.KpOwner) or "\n"
if redis:get(DEVIL.."Rank:Data") then
print("\027[" .. color.yellow[1] .. ";" .. color.black[2] .. "m" .. KPdata .. "\027[00m")
print("\027[" .. color.yellow[1] .. ";" .. color.black[2] .. "m" .. BotKPdata .. "\027[00m")
for v, user in pairs(config.Sudo_Users) do
local SudoData = redis:get(DEVIL.."KpSudos" .. user)
if SudoData then
...
redis:set(DEVIL.."Our_ID", result.id_)
end
...
local lists = redis:smembers(DEVIL..hashs)
redis:del(DEVIL.."SudoNumberGp" .. user_id)
for k, v in pairs(lists) do
redis:incr(DEVIL.."SudoNumberGp" .. user_id)
end
end
local setnumbergp_three = function(user_id)
local hashss = "sudo:data:" .. user_id
local lists = redis:smembers(DEVIL..hashss)
redis:del(DEVIL.."SudoNumberGp" .. user_id)
for k, v in pairs(lists) do
redis:incr(DEVIL.."SudoNumberGp" .. user_id)
end
end
local list = redis:smembers(DEVIL.."Bot:Admins")
for k, v in pairs(list) do
setnumbergp_two(v)
end
local lists = redis:smembers(DEVIL.."Bot:KpSudos")
for k, v in pairs(lists) do
setnumbergp_three(v)
end
redis:setex(DEVIL.."bot:reload", 7230, true)
...
local Bot_Channel = redis:get(DEVIL.."Bot:Channel") or tostring(_redis.Channel)
local sudo_users = _config.Sudo_Users
local Kp_Owner = redis:get(DEVIL.."Bot:KpOwnerBot")
local run = redis:get(DEVIL.."Bot:Run") or "True"
if not redis:get(DEVIL.."setnumbergp") then
setnumbergp()
redis:setex(DEVIL.."setnumbergp", 5 * hour, true)
end
...
local var = false
if redis:sismember(DEVIL.."Bot:KpSudos", msg.sender_user_id_) then
var = true
end
if msg.sender_user_id_ == tonumber(Kp_Owner) then
var = true
...
local var = false
if redis:sismember(DEVIL.."Bot:KpSudos", user_id) then
var = true
end
if user_id == tonumber(Kp_Owner) then
var = true
...
local admin = redis:sismember(DEVIL..hashsb, user_id)
if admin then
var = true
end
if redis:sismember(DEVIL.."Bot:KpSudos", user_id) then
...
local monshi = redis:sismember(DEVIL..hashssk, user_id)
local hashs = "Bot:Admins"
local admin = redis:sismember(DEVIL..hashs, user_id)
if monshi then
var = true
end
if admin then
...
local owner = redis:sismember(DEVIL..hash, user_id)
if monshi then
var = true
end
if admin then
...
local momod = redis:sismember(DEVIL..hash, user_id)
local hashs = "Bot:Admins"
local admin = redis:sismember(DEVIL..hashs, user_id)
local hashssk = "bot:monshis:" .. chat_id
local monshi = redis:sismember(DEVIL..hashssk, user_id)
local hashss = "bot:owners:" .. chat_id
local owner = redis:sismember(DEVIL..hashss, user_id)
local our_id = redis:get(DEVIL.."Our_ID") or 0
if momod then
var = true
end
if owner then
...
local vipmem = redis:sismember(DEVIL..hashsss, user_id)
if vipmem then
var = true
end
if momod then
...
local vipmems = redis:sismember(DEVIL..hashsss, user_id)
if vipmems then
var = true
end
if redis:sismember(DEVIL.."Bot:KpSudos", user_id) then
...
local banned = redis:sismember(DEVIL..hash, user_id)
if banned then
var = true
end
return var
...
local var = false
local hash = "bot:gban:"
local gbanned = redis:sismember(DEVIL..hash, user_id)
if gbanned then
var = true
end
return var
...
local muted = redis:sismember(DEVIL..hash, user_id)
local muted2 = redis:get(DEVIL..hash2)
if muted then
var = true
end
if muted2 then
...
if redis:get(DEVIL.."GetTime") then
local data = redis:get(DEVIL.."GetTime")
...
redis:setex(DEVIL.."GetTime", 10, url)
local A = jdat.FAtime
local B = jdat.FAdate
if A and B then
local T = {time = A, date = B}
...
local var = true
if redis:get(DEVIL.."Kpjoin1") then
local channel = ''..redis:get(DEVIL..'Kpch1')..''
local url , res = https.request('https://api.telegram.org/bot'..DEVIL_TOKEN..'/getchatmember?chat_id='..channel..'&user_id='..msg.sender_user_id_)
...
redis:set(DEVIL..'group:name'..GroupID,data.title_) end,nil) return redis:get(DEVIL..'group:name'..GroupID) end
...
link = redis:get(DEVIL.."bot:group:link" .. chat)
if link then
...
redis:del(DEVIL.."bot:muteall" .. chat_id)
redis:del(DEVIL.."bot:text:mute" .. chat_id)
redis:del(DEVIL.."bot:photo:mute" .. chat_id)
redis:del(DEVIL.."bot:video:mute" .. chat_id)
redis:del(DEVIL.."bot:selfvideo:mute" .. chat_id)
redis:del(DEVIL.."bot:gifs:mute" .. chat_id)
redis:del(DEVIL.."anti-flood:" .. chat_id)
redis:del(DEVIL.."flood:max:" .. chat_id)
redis:del(DEVIL.."bot:sens:spam" .. chat_id)
redis:del(DEVIL.."floodstatus" .. chat_id)
redis:del(DEVIL.."bot:music:mute" .. chat_id)
redis:del(DEVIL.."bot:bots:mute" .. chat_id)
redis:del(DEVIL.."bot:duplipost:mute" .. chat_id)
redis:del(DEVIL.."bot:inline:mute" .. chat_id)
redis:del(DEVIL.."bot:cmds" .. chat_id)
redis:del(DEVIL.."bot:bots:mute" .. chat_id)
redis:del(DEVIL.."bot:voice:mute" .. chat_id)
redis:del(DEVIL.."editmsg" .. chat_id)
redis:del(DEVIL.."bot:links:mute" .. chat_id)
redis:del(DEVIL.."bot:pin:mute" .. chat_id)
redis:del(DEVIL.."bot:sticker:mute" .. chat_id)
redis:del(DEVIL.."bot:tgservice:mute" .. chat_id)
redis:del(DEVIL.."bot:webpage:mute" .. chat_id)
redis:del(DEVIL.."bot:strict" .. chat_id)
redis:del(DEVIL.."bot:hashtag:mute" .. chat_id)
redis:del(DEVIL.."tags:lock" .. chat_id)
redis:del(DEVIL.."bot:location:mute" .. chat_id)
redis:del(DEVIL.."bot:contact:mute" .. chat_id)
redis:del(DEVIL.."bot:english:mute" .. chat_id)
redis:del(DEVIL.."bot:arabic:mute" .. chat_id)
redis:del(DEVIL.."bot:forward:mute" .. chat_id)
redis:del(DEVIL.."bot:member:lock" .. chat_id)
redis:del(DEVIL.."bot:document:mute" .. chat_id)
redis:del(DEVIL.."markdown:lock" .. chat_id)
redis:del(DEVIL.."Game:lock" .. chat_id)
redis:del(DEVIL.."bot:spam:mute" .. chat_id)
redis:del(DEVIL.."post:lock" .. chat_id)
redis:del(DEVIL.."bot:welcome" .. chat_id)
redis:del(DEVIL.."delidstatus" .. chat_id)
redis:del(DEVIL.."delpro:" .. chat_id)
redis:del(DEVIL.."sharecont" .. chat_id)
redis:del(DEVIL.."sayedit" .. chat_id)
redis:del(DEVIL.."welcome:" .. chat_id)
redis:del(DEVIL.."bot:group:link" .. chat_id)
redis:del(DEVIL.."bot:filters:" .. chat_id)
redis:del(DEVIL.."bot:muteall:Time" .. chat_id)
redis:del(DEVIL.."bot:muteall:start" .. chat_id)
redis:del(DEVIL.."bot:muteall:stop" .. chat_id)
redis:del(DEVIL.."bot:muteall:start_Unix" .. chat_id)
redis:del(DEVIL.."bot:muteall:stop_Unix" .. chat_id)
redis:del(DEVIL.."bot:muteall:Run" .. chat_id)
redis:del(DEVIL.."bot:muted:" .. chat_id)
end
...
redis:set(DEVIL.."bot:webpage:mute" .. chat_id, true)
redis:set(DEVIL.."bot:inline:mute" .. chat_id, true)
redis:set(DEVIL.."bot:bots:mute" .. chat_id, true)
redis:set(DEVIL.."tags:lock" .. chat_id, true)
redis:set(DEVIL.."markdown:lock" .. chat_id, true)
redis:set(DEVIL.."bot:links:mute" .. chat_id, true)
redis:set(DEVIL.."bot:hashtag:mute" .. chat_id, true)
redis:set(DEVIL.."bot:spam:mute" .. chat_id, true)
redis:set(DEVIL.."anti-flood:" .. chat_id, true)
redis:set(DEVIL.."Game:lock" .. chat_id, true)
redis:set(DEVIL.."bot:panel" .. chat_id, "one")
end
...
redis:set(DEVIL.."anti-flood:" .. chat_id, true)
redis:set(DEVIL.."Game:lock" .. chat_id, true)
redis:set(DEVIL.."post:lock" .. chat_id, true)
redis:set(DEVIL.."bot:forward:mute" .. chat_id, true)
redis:set(DEVIL.."bot:photo:mute" .. chat_id, true)
redis:set(DEVIL.."bot:video:mute" .. chat_id, true)
redis:set(DEVIL.."bot:selfvideo:mute" .. chat_id, true)
redis:set(DEVIL.."bot:gifs:mute" .. chat_id, true)
redis:set(DEVIL.."bot:sticker:mute" .. chat_id, true)
redis:set(DEVIL.."bot:location:mute" .. chat_id, true)
redis:set(DEVIL.."bot:document:mute" .. chat_id, true)
redis:set(DEVIL.."bot:panel" .. chat_id, "two")
end
...
redis:set(DEVIL.."bot:links:mute" .. chat_id, true)
redis:set(DEVIL.."bot:spam:mute" .. chat_id, true)
redis:set(DEVIL.."bot:sens:spam" .. chat_id, 500)
redis:set(DEVIL.."anti-flood:" .. chat_id, true)
redis:set(DEVIL.."Game:lock" .. chat_id, true)
redis:set(DEVIL.."bot:cmds" .. chat_id, true)
redis:set(DEVIL.."bot:duplipost:mute" .. chat_id, true)
redis:set(DEVIL.."bot:panel" .. chat_id, "three")
end
...
local user_id
local user = false
local list1 = redis:smembers(DEVIL.."Bot:KpSudos")
local list2 = redis:smembers(DEVIL.."Bot:Admins")
for k, v in pairs(list1) do
local hash = "sudo:data:" .. v
local is_add = redis:sismember(DEVIL..hash, chat)
if is_add then
user_id = v
end
end
...
if redis:sismember(DEVIL..hash, chat) then
user_id = Kp_Owner
end
if user_id then
local user_info = redis:get(DEVIL.."user:Name" .. user_id)
if user_info then
user = user_info
end
end
...
if hash then
local names = redis:hkeys(DEVIL..hash)
local text = ""
for i = 1, #names do
...
redis:set(DEVIL.."bot:Uptime", os.time())
----------------tdcli_update_callback---------------------------------------------------------------------------
function tdcli_update_callback(data)
local our_id = redis:get(DEVIL.."Our_ID") or 0
local api_id = redis:get(DEVIL.."Bot:Api_ID") or 0
if data.ID == "UpdateNewMessage" then
local msg = data.message_
local d = data.disable_notification_
local chat = chats[msg.chat_id_]
redis:sadd(DEVIL.."groups:users" .. msg.chat_id_, msg.sender_user_id_)--save users gp
redis:incr(DEVIL.."msgs:"..msg.sender_user_id_..":"..msg.chat_id_.."")--save msgs gp
if msg.content_.ID == "MessageChatDeleteMember" then
if tonumber(msg.content_.user_.id_) == tonumber(_redis.Bot_ID) then
local user_info_ = redis:get(DEVIL.."user:Name" .. msg.sender_user_id_)
local UserDEVIL = user_info_
if user_info_ then 
...
✓‏
‌‏]]
send(sudoed, 0, 1,iD_DEVIL, 1, "md")
redis:del(DEVIL.."bot:enable:" .. msg.chat_id_)
redis:srem(DEVIL.."bot:groups", msg.chat_id_)
end end end
if msg.content_.ID == "MessageChatAddMembers" then
redis:incr(DEVIL..'kpaddcon'..msg.chat_id_..':'..msg.sender_user_id_)
if msg.date_ < os.time() - 40 then
print("\027[" .. color.white[1] .. ";" .. color.magenta[2] .. "m>>> OLD MSG <<<\027[00m")
return false
end
if not redis:get(DEVIL.."Set_Our_ID") then
...
if not redis:get(DEVIL.."Rank:Data") then
...
redis:set(DEVIL.."KpSudos" .. user, "> Sudo User ID : " .. sudouserid .. [[
...
redis:set(DEVIL.."KpOwnerBot" .. Kp_Owner, "> Bot Owner ID : " .. botowneruserid .. [[
...
redis:set(DEVIL.."Botid" .. result.id_, "> Bot ID : " .. botuserid .. [[
...
redis:setex(DEVIL.."Rank:Data", 700, true)
end
if redis:get(DEVIL.."bot:reload") and 30 > tonumber(redis:ttl(DEVIL.."bot:reload")) then
load_config()
setnumbergp()
redis:setex(DEVIL.."bot:reload", 7230, true)
print("\027[" .. color.black[1] .. ";" .. color.green[2] .. "m>>> Bot Reloaded <<<\027[00m")
end
if not redis:get(DEVIL.."bot:reload2") then
redis:del(DEVIL.."bot:groups")
redis:del(DEVIL.."bot:userss")
redis:setex(DEVIL.."bot:reloadingtime", 22 * hour, true)
redis:setex(DEVIL.."bot:reload2", week, true)
redis:setex(DEVIL.."bot:reload3", 2222 * day, true)
redis:setex(DEVIL.."bot:reload4", 2222 * day, true)
end
if redis:get(DEVIL.."bot:reload3") and 500 >= tonumber(redis:ttl(DEVIL.."bot:reload3")) then
local hash = "bot:groups"
local list = redis:smembers(DEVIL..hash)
for k, v in pairs(list) do
if not redis:get(DEVIL.."bot:enable:" .. v) and not redis:get(DEVIL.."bot:charge:" .. v) then
resetgroup(v)
chat_leave(v, bot_id)
redis:srem(DEVIL..hash, v)
end
end
redis:del(DEVIL.."bot:reload3")
end
if redis:get(DEVIL.."bot:reload4") and redis:ttl(DEVIL.."bot:reload4") <= 600 then
local reload_data_sudo = function()
local hashsudo = "Bot:KpSudos"
local listsudo = redis:smembers(DEVIL..hashsudo)
for k, v in pairs(listsudo) do
local hashdata = "sudo:data:" .. v
local listdata = redis:smembers(DEVIL..hashdata)
for k, gp in pairs(listdata) do
if not redis:sismember(DEVIL.."bot:groups", gp) then
redis:srem(DEVIL..hashdata, gp)
end
end
end
end
...
local listadmin = redis:smembers(DEVIL..hashadmin)
...
------------------------------EXpirepannel GP ----------------------------------------------------------
local expiretime = redis:ttl(DEVIL.."bot:charge:" .. msg.chat_id_)
if not redis:get(DEVIL.."bot:charge:" .. msg.chat_id_) and redis:get(DEVIL.."bot:enable:" .. msg.chat_id_) then
redis:del(DEVIL.."bot:enable:" .. msg.chat_id_)
redis:srem(DEVIL.."bot:groups", msg.chat_id_)
end
if redis:get(DEVIL.."bot:charge:" .. msg.chat_id_) and not redis:get(DEVIL.."bot:enable:" .. msg.chat_id_) then
redis:set(DEVIL.."bot:enable:" .. msg.chat_id_, true)
end
if not redis:get(DEVIL.."bot:expirepannel:" .. msg.chat_id_) and redis:get(DEVIL.."bot:charge:" .. msg.chat_id_) and tonumber(expiretime) < tonumber(day) and tonumber(expiretime) >= 3600 then
local id = tostring(msg.chat_id_)
if id:match("-100(%d+)") then
local v = tonumber(Kp_Owner)
local list = redis:smembers(DEVIL.."bot:owners:" .. msg.chat_id_)
if list[1] or list[2] or list[3] or list[4] then
user_info = redis:get(DEVIL.."user:Name" .. (list[1] or list[2] or list[3] or list[4]))
end
if user_info then
owner = user_info
else
...
send(v, 0, 1, "💲┊ سوف تنتهي صلاحيه المجموعه\n🔅┊الرابط : " .. (redis:get(DEVIL.."bot:group:link" .. msg.chat_id_) or "لا يوجد ") .. "\n🚫┊ الايدي » " .. msg.chat_id_ .. "", 1, "html")
redis:setex(DEVIL.."bot:expirepannel:" .. msg.chat_id_, 43200, true)
end
end
------------------------Autoleave FOR BOT----------------------------------------------------
if redis:get(DEVIL.."autoleave") == "On" then
local id = tostring(msg.chat_id_)
if not redis:get(DEVIL.."bot:enable:" .. msg.chat_id_) and id:match("-100(%d+)") and not redis:get(DEVIL.."bot:autoleave:" .. msg.chat_id_) then
redis:setex(DEVIL.."bot:autoleave:" .. msg.chat_id_, 1400, true)
end
local autoleavetime = tonumber(redis:ttl(DEVIL.."bot:autoleave:" .. msg.chat_id_))
local time = 400
if tonumber(autoleavetime) < tonumber(time) and tonumber(autoleavetime) > 150 then
redis:set(DEVIL.."lefting" .. msg.chat_id_, true)
end
local id = tostring(msg.chat_id_)
if id:match("-100(%d+)") and redis:get(DEVIL.."lefting" .. msg.chat_id_) then
if not redis:get(DEVIL.."bot:enable:" .. msg.chat_id_) and not redis:get(DEVIL.."bot:charge:" .. msg.chat_id_) then
redis:del(DEVIL.."lefting" .. msg.chat_id_)
redis:del(DEVIL.."bot:autoleave:" .. msg.chat_id_)
chat_leave(msg.chat_id_, bot_id)
local v = tonumber(Kp_Owner)
send(v, 0, 1, "💲┊ تم مغادره المجموعــه\n🔱┊ الاسم » 👇🏾\n🏮┊ ("..title_name(msg.chat_id_)..")\n🚫┊ الايدي » " .. msg.chat_id_, 1, "html")
redis:srem(DEVIL.."bot:groups", msg.chat_id_)
elseif redis:get(DEVIL.."bot:enable:" .. msg.chat_id_) then
redis:del(DEVIL.."lefting" .. msg.chat_id_)
end
end
elseif redis:get(DEVIL.."bot:charge:" .. msg.chat_id_) == "Trial" and 500 > redis:ttl(DEVIL.."bot:charge:" .. msg.chat_id_) then
local v = tonumber(Kp_Owner)
send(v, 0, 1, "💲┊ تم مغادره المجموعــه\n🔱┊ الاسم » ("..title_name(msg.chat_id_)..")\n🚫┊ الايدي » " .. msg.chat_id_, 1, "html")
redis:srem(DEVIL.."bot:groups", msg.chat_id_)
chat_leave(msg.chat_id_, bot_id)
redis:del(DEVIL.."bot:charge:" .. msg.chat_id_)
end
...
redis:set(DEVIL..svgroup, chatname)
end
end
...
local id = result.id_
if username then
redis:set(DEVIL..svuser, "@" .. username)
else
redis:set(DEVIL..svuser, name)
end
end
getUser(msg.sender_user_id_, check_username)
if redis:get(DEVIL.."clerk") == "On" then
local clerk = function(extra, result)
if not is_admin(result.id_) then
local textc = redis:get(DEVIL.."textsec")
if not redis:get(DEVIL.."secretary_:" .. msg.chat_id_) and textc then
...
redis:setex(DEVIL.."secretary_:" .. msg.chat_id_, day, true)
end
end
end
...
local status_welcome = (redis:get(DEVIL..'status:welcome:'..msg.chat_id_) or 'disable')
...
if redis:get(DEVIL..'welcome:'..msg.chat_id_) then
text = redis:get(DEVIL..'welcome:'..msg.chat_id_)
else
...
----------------- save all msg bot --------------------------
redis:incr(DEVIL.."bot:allmsgs")
if msg.chat_id_ then
local id = tostring(msg.chat_id_)
if id:match("-100(%d+)") then
if not redis:sismember(DEVIL.."bot:groups", msg.chat_id_) then
redis:sadd(DEVIL.."bot:groups", msg.chat_id_)
end
elseif id:match("^(%d+)") then
if not redis:sismember(DEVIL.."bot:userss", msg.chat_id_) then
redis:sadd(DEVIL.."bot:userss", msg.chat_id_)
end
elseif not redis:sismember(DEVIL.."bot:groups", msg.chat_id_) then
redis:sadd(DEVIL.."bot:groups", msg.chat_id_)
end
end
---------------type the msg--------------------------------------------------
...
redis:incr(DEVIL.."text:"..msg.sender_user_id_..":"..msg.chat_id_.."")
text = msg.content_.text_
print("\027[" .. color.black[1] .. ";" .. color.yellow[2] .. "m>> [ Text ] <<\027[00m")
msg_type = "MSG:Text"
end
if msg.content_.ID == "MessagePhoto" then
redis:incr(DEVIL.."Photo:"..msg.sender_user_id_..":"..msg.chat_id_.."")
...
redis:incr(DEVIL.."sticker:"..msg.sender_user_id_..":"..msg.chat_id_.."")
if not redis:get(DEVIL.."lock_STCK"..msg.chat_id_) then
if not redis:get(DEVIL..'lock:add'..msg.chat_id_) then
local DEVIL = {"منور انت ، 😙","اه باع وجهك شكد نضيف 😅","هذا منو ، 😏","تسمحلي ابوسك ☹️😹","مليان ضحك مليان 😹❤️","تف على هذا  ويهك 💦😹","اذا حاته ممكن الرقم 😆😹","تدري صار "..(redis:get(DEVIL.."sticker:"..msg.sender_user_id_..":"..msg.chat_id_.."")).." ملصق داز  شهالتبذير 🤔😹","كافي ملصقات مشايف 😫"}
send(msg.chat_id_, msg.id_, 1,""..DEVIL[math.random(#DEVIL)].."", 1, 'md')
end
end
...
redis:incr(DEVIL.."Voice:"..msg.sender_user_id_..":"..msg.chat_id_.."")
print("\027[" .. color.black[1] .. ";" .. color.yellow[2] .. "m>> [ Voice ] <<\027[00m")
msg_type = "MSG:Voice"
end
if msg.content_.ID == "MessageVideo" then
redis:incr(DEVIL.."Video:"..msg.sender_user_id_..":"..msg.chat_id_.."")
print("\027[" .. color.black[1] .. ";" .. color.yellow[2] .. "m>> [ Video ] <<\027[00m")
msg_type = "MSG:Video"
end
if msg.content_.ID == "MessageAnimation" then
redis:incr(DEVIL.."Gif:"..msg.sender_user_id_..":"..msg.chat_id_.."")
...
redis:incr(DEVIL.."SelfVideo:"..msg.sender_user_id_..":"..msg.chat_id_.."")
...
local content_text = redis:get(DEVIL..'add:repgp'..msg.sender_user_id_..''..msg.chat_id_..'')
if content_text == 'save_repgp' then
redis:del(DEVIL..'add:repgp'..msg.sender_user_id_..''..msg.chat_id_..'')
local content_text = redis:get(DEVIL..'addreplaygp:'..msg.sender_user_id_..''..msg.chat_id_..'')
if msg.content_.video_ then
redis:set(DEVIL..'video_repgp'..content_text..''..msg.chat_id_..'', msg.content_.video_.video_.persistent_id_)
end
if msg.content_.sticker_ then
redis:set(DEVIL..'stecker_repgp'..content_text..''..msg.chat_id_..'', msg.content_.sticker_.sticker_.persistent_id_)
end
if msg.content_.voice_ then
redis:set(DEVIL..'voice_repgp'..content_text..''..msg.chat_id_..'', msg.content_.voice_.voice_.persistent_id_)
end
if msg.content_.animation_ then
redis:set(DEVIL..'gif_repgp'..content_text..''..msg.chat_id_..'', msg.content_.animation_.animation_.persistent_id_)
end
if msg.content_.text_ then
redis:set(DEVIL..'text_repgp'..content_text..''..msg.chat_id_..'', msg.content_.text_)
end
redis:sadd('rep_owner'..msg.chat_id_..'',content_text)
send(msg.chat_id_, msg.id_, 1, "📌┊ تم حفظ الرد بنجاح\n", 1, 'md')
redis:del(DEVIL..'addreplaygp:'..msg.sender_user_id_..''..msg.chat_id_..'')
return false end end
if msg.content_.text_ and not redis:get(DEVIL..'lock_reeeep'..msg.chat_id_) then
if redis:get(DEVIL..'video_repgp'..msg.content_.text_..''..msg.chat_id_..'') then
sendVideo(msg.chat_id_, msg.id_, 0, 1,nil, redis:get(DEVIL..'video_repgp'..msg.content_.text_..''..msg.chat_id_..''))
end
if redis:get(DEVIL..'voice_repgp'..msg.content_.text_..''..msg.chat_id_..'')  then
sendVoice(msg.chat_id_, msg.id_, 0, 1, nil, redis:get(DEVIL..'voice_repgp'..msg.content_.text_..''..msg.chat_id_..''))
end
if  redis:get(DEVIL..'gif_repgp'..msg.content_.text_..''..msg.chat_id_..'') then
sendDocument(msg.chat_id_, msg.id_, 0, 1, nil, redis:get(DEVIL..'gif_repgp'..msg.content_.text_..''..msg.chat_id_..''))
end
if redis:get(DEVIL..'stecker_repgp'..msg.content_.text_..''..msg.chat_id_..'') then
sendSticker(msg.chat_id_, msg.id_, 0, 1,nil, redis:get(DEVIL..'stecker_repgp'..msg.content_.text_..''..msg.chat_id_..''))
end
if redis:get(DEVIL..'text_repgp'..msg.content_.text_..''..msg.chat_id_..'') then
send(msg.chat_id_, msg.id_, 1, redis:get(DEVIL..'text_repgp'..msg.content_.text_..''..msg.chat_id_..'') ,  1, 'md')
end
end
---------------------------------in all gps---------------------------------------------------
text = msg.content_.text_
if msg.content_.text_  or msg.content_.video_ or msg.content_.sticker_ or msg.content_.voice_ or msg.content_.animation_ then
local content_text = redis:get(DEVIL.."add:repallt"..msg.sender_user_id_)
if content_text == 'save_rep' then
redis:del(DEVIL.."add:repallt"..msg.sender_user_id_)
local content_text = redis:get(DEVIL.."addreply2:"..msg.sender_user_id_)
if msg.content_.video_ then
redis:set(DEVIL.."video_repall"..content_text, msg.content_.video_.video_.persistent_id_)
end
if msg.content_.sticker_ then
redis:set(DEVIL.."stecker_repall"..content_text, msg.content_.sticker_.sticker_.persistent_id_)
end
if msg.content_.voice_ then
redis:set(DEVIL.."voice_repall"..content_text, msg.content_.voice_.voice_.persistent_id_)
end
if msg.content_.animation_ then
redis:set(DEVIL.."gif_repall"..content_text, msg.content_.animation_.animation_.persistent_id_)
end
if msg.content_.text_ then
redis:set(DEVIL.."text_repall"..content_text, msg.content_.text_)
end
redis:sadd('rep_sudo',content_text)
send(msg.chat_id_, msg.id_, 1, "📌┊ تم حفظ الرد بنجاح\n", 1, 'md')
redis:del(DEVIL.."addreply2:"..msg.sender_user_id_)
return false end end
if msg.content_.text_ and not redis:get(DEVIL..'lock_reeeep'..msg.chat_id_) then
if redis:get(DEVIL.."video_repall"..msg.content_.text_) then
sendVideo(msg.chat_id_, msg.id_, 0, 1,nil, redis:get(DEVIL.."video_repall"..msg.content_.text_))
end
if redis:get(DEVIL.."voice_repall"..msg.content_.text_)  then
sendVoice(msg.chat_id_, msg.id_, 0, 1, nil, redis:get(DEVIL.."voice_repall"..msg.content_.text_))
end
if  redis:get(DEVIL.."gif_repall"..msg.content_.text_) then
sendDocument(msg.chat_id_, msg.id_, 0, 1, nil, redis:get(DEVIL.."gif_repall"..msg.content_.text_))
end
if redis:get(DEVIL.."stecker_repall"..msg.content_.text_) then
sendSticker(msg.chat_id_, msg.id_, 0, 1,nil, redis:get(DEVIL.."stecker_repall"..msg.content_.text_))
end
if redis:get(DEVIL.."text_repall"..msg.content_.text_) then
send(msg.chat_id_, msg.id_, 1, redis:get(DEVIL.."text_repall"..msg.content_.text_) ,  1, "md")
end
end
...
if not redis:get(DEVIL..flmax) then
floodMax = 5
else
floodMax = tonumber(redis:get(DEVIL..flmax))
end
local pm = "flood:" .. msg.sender_user_id_ .. ":" .. msg.chat_id_ .. ":msgs"
if not redis:get(DEVIL..pm) then
msgs = 0
else
msgs = tonumber(redis:get(DEVIL..pm))
end
local TIME_CHECK = 2
local TIME_CHECK_PV = 2
local hashflood = "anti-flood:" .. msg.chat_id_
if msgs > floodMax - 1 then
if redis:get(DEVIL.."floodstatus" .. msg.chat_id_) == "Kicked" then
del_all_msgs(msg.chat_id_, msg.sender_user_id_)
chat_kick(msg.chat_id_, msg.sender_user_id_)
elseif redis:get(DEVIL.."floodstatus" .. msg.chat_id_) == "DelMsg" then
del_all_msgs(msg.chat_id_, msg.sender_user_id_)
else
del_all_msgs(msg.chat_id_, msg.sender_user_id_)
end
...
if not redis:get(DEVIL..pmonpv) then
msgsonpv = 0
else
msgsonpv = tonumber(redis:get(DEVIL..pmonpv))
end
if msgsonpv > 12 then
blockUser(msg.sender_user_id_)
end
local idmem = tostring(msg.chat_id_)
if idmem:match("^(%d+)") and not is_admin(msg.sender_user_id_) and not redis:get(DEVIL.."Filtering:" .. msg.sender_user_id_) then
redis:setex(DEVIL..pmonpv, TIME_CHECK_PV, msgsonpv + 1)
end
...
if redis:get(DEVIL.."keed_link"..msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
local chat = msg.chat_id_
delete_msg(chat, msgs)
HTTPS.request("https://api.telegram.org/bot" .. DEVIL_TOKEN .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false")
redis:sadd(DEVIL..'bot:keed:'..msg.chat_id_, msg.sender_user_id_)
...
if redis:get(DEVIL.."keed_fosh"..msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(DEVIL.."keed_user"..msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if msg.content_.ID == "MessagePhoto" then
if redis:get(DEVIL.."keed_photo" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(DEVIL.."keed_text"..msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if text and text:match("(.*)(#)(.*)")  then
if redis:get(DEVIL.."keed_hashtag"..msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(DEVIL.."keed_fwd"..msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if msg.content_.ID == "MessageSticker" then
if redis:get(DEVIL.."keed_stecker"..msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if msg.content_.ID == "MessageAudio" then
if redis:get(DEVIL.."keed_audio"..msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if msg.content_.ID == "MessageVoice" then
if redis:get(DEVIL.."keed_voice"..msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if msg.content_.ID == "MessageVideo" then
if redis:get(DEVIL.."keed_video"..msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if msg.content_.ID == "MessageAnimation" then
if redis:get(DEVIL.."keed_gif"..msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if msg.content_.ID == "MessageContact" then
if redis:get(DEVIL.."keed_contect"..msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(DEVIL.."keed_arbic"..msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if msg.content_.ID == "MessageDocument" then
if redis:get(DEVIL.."keed_Document"..msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(DEVIL.."keed_english"..msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(DEVIL.."keed_markdon"..msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if not redis:get(DEVIL.."bot:muted:Time" .. msg.chat_id_ .. ":" .. msg.sender_user_id_) and redis:sismember(DEVIL.."bot:muted:" .. msg.chat_id_, msg.sender_user_id_) then
redis:srem(DEVIL.."bot:muted:" .. msg.chat_id_, msg.sender_user_id_)
end
if is_gbanned(msg.sender_user_id_) then
chat_kick(msg.chat_id_, msg.sender_user_id_)
return
end
if redis:get(DEVIL.."bot:muteall:Time" .. msg.chat_id_) then
local start_ = redis:get(DEVIL.."bot:muteall:start" .. msg.chat_id_)
local start = start_:gsub(":", "")
local stop_ = redis:get(DEVIL.."bot:muteall:stop" .. msg.chat_id_)
local stop = stop_:gsub(":", "")
local SVTime = os.date("%R")
local Time = SVTime:gsub(":", "")
if tonumber(Time) >= tonumber(start) and not redis:get(DEVIL.."bot:muteall:Run" .. msg.chat_id_) then
local g = os.time()
redis:set(DEVIL.."bot:muteall:start_Unix" .. msg.chat_id_, g)
...
redis:set(DEVIL.."bot:muteall:stop_Unix" .. msg.chat_id_, unix)
redis:set(DEVIL.."bot:muteall:Run" .. msg.chat_id_, true)
end
end
if redis:get(DEVIL.."bot:muteall:Time" .. msg.chat_id_) and redis:get(DEVIL.."bot:muteall:Run" .. msg.chat_id_) then
local SR = redis:get(DEVIL.."bot:muteall:start_Unix" .. msg.chat_id_) or 0
local SP = redis:get(DEVIL.."bot:muteall:stop_Unix" .. msg.chat_id_) or 0
...
redis:del(DEVIL.."bot:muteall:Run" .. msg.chat_id_)
end
end
---------------------pinnedmsg----------------------------------------------------------
if msg.content_.ID == "MessagePinMessage" and not msg.sender_user_id_ == our_id and not is_owner(msg.sender_user_id_, msg.chat_id_) and redis:get(DEVIL.."pinnedmsg" .. msg.chat_id_) and redis:get(DEVIL.."bot:pin:mute" .. msg.chat_id_) then
unpinmsg(msg.chat_id_)
local pin_id = redis:get(DEVIL.."pinnedmsg" .. msg.chat_id_)
pinmsg(msg.chat_id_, pin_id, 0)
end
if not redis:get(DEVIL.."Resetdatapost" .. msg.chat_id_) then
redis:del(DEVIL.."Gp:Post" .. msg.chat_id_)
redis:setex(DEVIL.."Resetdatapost" .. msg.chat_id_, 12 * hour, true)
end
----------------------com viewget -----------------------------------------------------
if redis:get(DEVIL.."bot:viewget" .. msg.sender_user_id_) then
if not msg.forward_info_ then
send(msg.chat_id_, msg.id_, 1, "✯↓ حدث خطا حاول مره اخرى", 1, "md")
redis:del(DEVIL.."bot:viewget" .. msg.sender_user_id_)
else
send(msg.chat_id_, msg.id_, 1, "📛↓ عـــدد المشاهـــدات \n: " .. msg.views_ .. "", 1, "md")
redis:del(DEVIL.."bot:viewget" .. msg.sender_user_id_)
end end
if redis:get(DEVIL.."bot:viewmsg") == "On" then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(DEVIL..'bot:setphoto'..msg.chat_id_..':'..msg.sender_user_id_) then
...
redis:del(DEVIL..'bot:setphoto'..msg.chat_id_..':'..msg.sender_user_id_)
setphoto(msg.chat_id_, photo_id)
end
------------------------------------------------------------------------------------------------
if redis:get(DEVIL.."broadcast" .. msg.chat_id_ .. ":" .. msg.sender_user_id_) then
local gps = redis:scard(DEVIL.."bot:groups") or 0
local gpss = redis:smembers(DEVIL.."bot:groups") or 0
local id = msg.id_
local msgs = {
[0] = id
}
...
redis:del(DEVIL.."broadcast" .. msg.chat_id_ .. ":" .. msg.sender_user_id_)
end
if not is_vipmem(msg.sender_user_id_, msg.chat_id_) then
if redis:get(DEVIL.."anti-flood:" .. msg.chat_id_) then
redis:setex(DEVIL..pm, TIME_CHECK, msgs + 1)
end
--«««««««««««««««««««««««« Developer By Karrar DEVIL »»»»»»»»»»»»»»»»»»»»»»»»»»»--
if msg.content_.caption_ then
...
if redis:get(DEVIL.."bot:duplipost:mute" .. msg.chat_id_) then
if redis:sismember(DEVIL.."Gp:Post" .. msg.chat_id_, post) then
local id = msg.id_
local msgs = {
[0] = id
}
...
redis:sadd(DEVIL.."Gp:Post" .. msg.chat_id_, post)
end
end
if redis:get(DEVIL.."post:lock" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if msg.forward_info_ and redis:get(DEVIL.."bot:forward:mute" .. msg.chat_id_) and (msg.forward_info_.ID == "MessageForwardedFromUser" or msg.forward_info_.ID == "MessageForwardedPost") then
local id = msg.id_
local msgs = {
[0] = id
}
...
if msg.content_.ID == "MessagePhoto" then
if redis:get(DEVIL.."bot:photo:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(DEVIL.."bot:links:mute" .. msg.chat_id_) and (msg.content_.caption_:match("[Tt][Ee][Ll][Ee][Gg][Rr][Aa][Mm].[Mm][Ee]") or msg.content_.caption_:match("[Tt][Ll][Gg][Rr][Mm].[Mm][Ee]") or msg.content_.caption_:match("[Tt].[Mm][Ee]")) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(DEVIL.."bot:strict" .. msg.chat_id_) then
chat_kick(msg.chat_id_, msg.sender_user_id_)
end
end
if msg.content_.caption_:match("@") then
if redis:get(DEVIL.."tags:lock" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
elseif msg.content_.entities_ and msg.content_.entities_[0] and msg.content_.entities_[0].ID == "MessageEntityMention" and redis:get(DEVIL.."tags:lock" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if msg.content_.caption_:match("#") then
if redis:get(DEVIL.."bot:hashtag:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
elseif msg.content_.entities_ and msg.content_.entities_[0] and msg.content_.entities_[0].ID == "MessageEntityHashtag" and redis:get(DEVIL.."bot:hashtag:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(DEVIL.."bot:webpage:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
elseif msg.content_.entities_ and msg.content_.entities_[0] and (msg.content_.entities_[0].ID == "MessageEntityTextUrl" or msg.content_.entities_[0].ID == "MessageEntityUrl") and redis:get(DEVIL.."bot:webpage:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if msg.content_.web_page_ and redis:get(DEVIL.."bot:webpage:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if msg.content_.caption_:match("[\216-\219][\128-\191]") and redis:get(DEVIL.."bot:arabic:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if (msg.content_.caption_:match("[A-Z]") or msg.content_.caption_:match("[a-z]")) and redis:get(DEVIL.."bot:english:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(DEVIL.."markdown:lock" .. msg.chat_id_) and not is_vipmem(msg.sender_user_id_, msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(DEVIL.."Game:lock" .. msg.chat_id_) and not is_vipmem(msg.sender_user_id_, msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if not is_vipmem(msg.sender_user_id_, msg.chat_id_) and redis:get(DEVIL.."tags:lock" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(DEVIL.."bot:document:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if msg.content_.caption_:match("[\216-\219][\121-\191]") and redis:get(DEVIL.."bot:arabic:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(DEVIL.."bot:inline:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(DEVIL.."clerk") == "On" or is_admin(msg.sender_user_id_) then
...
if redis:get(DEVIL.."bot:sticker:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(DEVIL.."bot:tgservice:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if not is_vipmem(msg.sender_user_id_, msg.chat_id_) and redis:get(DEVIL.."bot:member:lock" .. msg.chat_id_) then
chat_kick(msg.chat_id_, msg.sender_user_id_)
end
...
for i = 0, #list do
if list[i].type_.ID == "UserTypeBot" and not is_momod(list[i].id_, msg.chat_id_) and redis:get(DEVIL.."bot:bots:mute" .. msg.chat_id_) then
chat_kick(msg.chat_id_, list[i].id_)
end
if list[i].type_.ID == "UserTypeBot" and not is_momod(list[i].id_, msg.chat_id_) and redis:get(DEVIL.."bot:botskick" .. msg.chat_id_) then
chat_kick(msg.chat_id_, list[i].id_)
chat_kick(msg.chat_id_, msg.sender_user_id_)
local user_info_ = redis:get(DEVIL..'user:Name' .. msg.sender_user_id_)
local UserDEVIL = user_info_
if user_info_ then
send(msg.chat_id_, msg.id_, 1, "👨‍✈️┊ العضو » (["..UserDEVIL.."])\n🚫┊ الايدي » (*"..msg.sender_user_id_.."*)\n🗯┊ قام بأضافه بوت في المجموعه\n📌┊ تم طرد البوت مع العضو \n✓", 1, "md")
end 
end
if list[i].type_.ID == "UserTypeBot" and not is_momod(list[i].id_, msg.chat_id_) and redis:get(DEVIL.."keed_bots"..msg.chat_id_) then
chat_kick(msg.chat_id_, list[i].id_)
redis:sadd(DEVIL..'bot:keed:'..msg.chat_id_, msg.sender_user_id_)
HTTPS.request("https://api.telegram.org/bot" .. DEVIL_TOKEN .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false")
redis:sadd(DEVIL..'bot:keed:'..msg.chat_id_, msg.sender_user_id_)
...
if redis:get(DEVIL.."bot:member:lock" .. msg.chat_id_) and not is_vipmem(msg.content_.members_[0].id_, msg.chat_id_) and not is_vipmem(msg.sender_user_id_, msg.chat_id_) then
...
if redis:get(DEVIL.."bot:contact:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(DEVIL.."bot:music:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(DEVIL.."bot:voice:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(DEVIL.."bot:location:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(DEVIL.."bot:video:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(DEVIL.."bot:selfvideo:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(DEVIL.."bot:gifs:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if msg.content_.caption_ then
check_filter_words(msg, msg.content_.caption_)
if redis:get(DEVIL.."bot:links:mute" .. msg.chat_id_) and (msg.content_.caption_:match("[Hh][Tt][Tt][Pp][Ss]://") or msg.content_.caption_:match("[Hh][Tt][Tt][Pp]://")) then
local id = msg.id_
local msgs = {
[0] = id
}
...
redis:setex(DEVIL.."bot:editid" .. msg.id_, day, msg.content_.text_)
if redis:get(DEVIL.."anti-flood:" .. msg.chat_id_) and not is_vipmem(msg.sender_user_id_, msg.chat_id_) then
redis:setex(DEVIL..pm, TIME_CHECK, msgs + 1)
end
---------------------rem cam filters---------------------------------------------------------------------
if redis:get(DEVIL.."Filtering:" .. msg.sender_user_id_) then
local chat = redis:get(DEVIL.."Filtering:" .. msg.sender_user_id_)
...
---------------save name bot-----------------------------
if redis:get(DEVIL..'botts:namess'..msg.sender_user_id_) then
redis:del(DEVIL..'botts:namess'..msg.sender_user_id_)
local NAME_BOT = msg.content_.text_:match("(.*)")
redis:set(DEVIL..'DEVILnams',NAME_BOT)
send(msg.chat_id_, msg.id_, 1, "🌀┊ تم وضع اسم البوت 🍃",1, 'html')
return false
end
------------------------save cam link-----------------------------------
if redis:get(DEVIL.."bot:group:link" .. msg.chat_id_ .. ":" .. msg.sender_user_id_) and (msg.content_.text_:match("(https://telegram.me/joinchat/%S+)") or msg.content_.text_:match("(https://t.me/joinchat/%S+)")) then
local glink = msg.content_.text_:match("(https://telegram.me/joinchat/%S+)") or msg.content_.text_:match("(https://t.me/joinchat/%S+)")
local hash = "bot:group:link" .. msg.chat_id_
redis:set(DEVIL..hash, glink)
send(msg.chat_id_, msg.id_, 1, "🌀┊ تـــم  حفــــظ الرابط 🎈", 1, "md")
redis:del(DEVIL.."bot:group:link" .. msg.chat_id_ .. ":" .. msg.sender_user_id_)
end
--------------------------------------------------------------------------
if redis:get(DEVIL.."gettextsec" .. msg.sender_user_id_) then
local clerktext = msg.content_.text_
redis:set(DEVIL.."textsec", clerktext)
send(msg.chat_id_, msg.id_, 1, "🌀┊  تم حفظ الكليشه 🎈", 1, "md")
redis:del(DEVIL.."gettextsec" .. msg.sender_user_id_)
end
---------------------save com rules-----------------------------------------------------------------------------
if redis:get(DEVIL.."rules" .. msg.chat_id_ .. ":" .. msg.sender_user_id_) then
local rules = msg.content_.text_
redis:set(DEVIL.."bot:rules" .. msg.chat_id_, rules)
send(msg.chat_id_, msg.id_, 1, "🌀┊  تـــم حفــــظ القوانين 🎋",  1, "md")
redis:del(DEVIL.."rules" .. msg.chat_id_ .. ":" .. msg.sender_user_id_)
end
-----------ch join------------------------------------------------------
if redis:get(DEVIL..'Kpch'..msg.sender_user_id_) then
redis:del(DEVIL..'Kpch'..msg.sender_user_id_)
local url , res = https.request('https://api.telegram.org/bot'..DEVIL_TOKEN..'/getChatAdministrators?chat_id='..msg.content_.text_..'')
local data = KPJS:decode(url)
if res == 400 then
...
--------------------rem  cam broadcast2--------------------------------------------------------------------------
if redis:get(DEVIL.."broadcast2" .. msg.chat_id_ .. ":" .. msg.sender_user_id_) then
if text:match("^الغاء$") then
send(msg.chat_id_, msg.id_, 1, "🔰- تم الغاء الامر بنجاح🎈 ", 1, "md")
redis:del(DEVIL.."broadcast2" .. msg.chat_id_ .. ":" .. msg.sender_user_id_)
else
local gps = redis:scard(DEVIL.."bot:groups") or 0
local gpss = redis:smembers(DEVIL.."bot:groups") or 0
local msgs = {
...
if redis:get(DEVIL.."bot:joinbylink:" .. msg.sender_user_id_) and (msg.content_.text_:match("(https://telegram.me/joinchat/%S+)") or msg.content_.text_:match("(https://t.me/joinchat/%S+)")) then
else
end
--------------------set cam sudo  ------------------------------------------
if redis:get(DEVIL.."bot:DEVIL_dev" .. msg.chat_id_ .. ":" .. msg.sender_user_id_) then
redis:del(DEVIL.."bot:DEVIL_dev" .. msg.chat_id_ .. ":" .. msg.sender_user_id_)
local DEVIL_dev = msg.content_.text_:match("(.*)")
redis:set(DEVIL.."DEVIL_dev", DEVIL_dev)
...
if (text:match("[Tt][Ee][Ll][Ee][Gg][Rr][Aa][Mm].[Mm][Ee]") or text:match("[Tt][Ll][Gg][Rr][Mm].[Mm][Ee]") or text:match("[Tt].[Mm][Ee]")) and redis:get(DEVIL.."bot:links:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(DEVIL.."bot:text:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if msg.content_.text_:match("[\216-\219][\121-\191]") and redis:get(DEVIL.."bot:arabic:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if not redis:get(DEVIL..hash) then
sens = 400
else
sens = tonumber(redis:get(DEVIL..hash))
end
if redis:get(DEVIL.."bot:spam:mute" .. msg.chat_id_) and string.len(msg.content_.text_) > sens or ctrl_chars > sens or real_digits > sens then
delete_msg(chat, msgs)
print_del_msg("Deleted Because [Lock] [Spam] ")
end
end
if (msg.content_.text_:match("[A-Z]") or msg.content_.text_:match("[a-z]")) and redis:get(DEVIL.."bot:english:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(DEVIL.."bot:cmds" .. msg.chat_id_) and not is_momod(msg.sender_user_id_, msg.chat_id_) then
...
-------------------------------leave groups----------------------------------------------------------------------
if is_sudo(msg) and idf:match("-100(%d+)") and (text:match('^'..(redis:get(DEVIL..'DEVILnams') or 'DEVIL')..' غادر$')) then
...
redis:set(DEVIL.."DEVIL_O" .. msg.chat_id_, uuuu[2])
end
local msgs = tonumber(redis:get(DEVIL.."msgs:"..msg.sender_user_id_..":"..msg.chat_id_))
local get_DEVIL = tonumber(redis:get(DEVIL.."DEVIL_O" .. msg.chat_id_)) 
if msgs == get_DEVIL and not redis:get(DEVIL.."lock_addd"..msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ هذا العضو اثبت تفاعله\n🎟┊تم رفعه ادمن في المجموعه\n✓",  1, "md")
redis:sadd(DEVIL..'bot:momod:'..msg.chat_id_, msg.sender_user_id_)
end
--------------------------------------------------------------------
...
send(msg.chat_id_, msg.id_, 1, "⚜️┊ تم حفظ العدد *"..uuuu[2].."*\n💬┊ سيتم رفع العضو مميز\n🚫┊اذا اثبت تفاعله\n✓",1, 'md')
redis:set(DEVIL.."DEVIL_OO" .. msg.chat_id_, uuuu[2])
end
local msgs = tonumber(redis:get(DEVIL.."msgs:"..msg.sender_user_id_..":"..msg.chat_id_))
local get_DEVIL = tonumber(redis:get(DEVIL.."DEVIL_OO" .. msg.chat_id_)) 
if msgs == get_DEVIL and not redis:get(DEVIL.."lock_adddvip"..msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "📌┊ هذا العضو اثبت تفاعله\n🌀┊تم رفعه عضو مميز في المجموعه\n✓",  1, "md")
redis:sadd(DEVIL..'bot:vipmem:'..msg.chat_id_, msg.sender_user_id_)
end
------------------------------ADD vipmems BY Reply------------------------------------------------------------------
if text:match('^رفع مميز عام$') and is_KP(msg) and msg.reply_to_message_id_ ~= 0  then
function promote_by_reply(extra, result, success)
local user_info_ = redis:get(DEVIL..'user:Name' .. result.sender_user_id_)
local UserDEVIL = user_info_
if user_info_ then
local hash = 'bot:vipmems:'
if redis:sismember(DEVIL..hash, result.sender_user_id_) then
send(msg.chat_id_, msg.id_, 1,'👨‍✈️┊ العضو » (['..UserDEVIL..'])\n⚠️┊ مرفوع مميز عام سابقا\n✓‏', 1, 'md')
else
redis:sadd(DEVIL..hash, result.sender_user_id_)
send(msg.chat_id_, msg.id_, 1,'👨‍✈️┊ العضو » (['..UserDEVIL..'])\n⚠️┊ تم رفعه مميز عام\n✓', 1, 'md')
end end end
...
local hash = 'bot:vipmems:'
if result.id_ then
if redis:sismember(DEVIL..hash, result.id_) then
send(msg.chat_id_, msg.id_, 1,'👨‍✈️┊ العضو » ([@'..ap[2]..'])\n⚠️┊ مرفوع مميز عام سابقا\n✓‏', 1, 'md')
else
redis:sadd(DEVIL..hash, result.id_)
send(msg.chat_id_, msg.id_, 1,'👨‍✈️┊ العضو » ([@'..ap[2]..'])\n⚠️┊ تم رفعه مميز عام\n✓‏', 1, 'md')
end end end
...
local user_info_ = redis:get(DEVIL..'user:Name' .. ap[2])
local UserDEVIL = user_info_
if user_info_ then
local hash = 'bot:vipmems:'
if redis:sismember(DEVIL..hash, ap[2]) then
send(msg.chat_id_, msg.id_, 1,'👨‍✈️┊ العضو » (['..UserDEVIL..'])\n⚠️┊ مرفوع مميز عام سابقا\n✓‏', 1, 'md')
else
redis:sadd(DEVIL..hash, ap[2])
send(msg.chat_id_, msg.id_, 1,'👨‍✈️┊ العضو » (['..UserDEVIL..'])\n⚠️┊ تم رفعه مميز عام\n✓‏', 1, 'md')
end end end
...
if user_info_ then
local hash = 'bot:vipmems:'
if not redis:sismember(DEVIL..hash, result.sender_user_id_) then
send(msg.chat_id_, msg.id_, 1,'👨‍✈️┊ العضو » (['..UserDEVIL..'])\n⚠️┊ تم تنزيله مميز عتم سابقا\n‏', 1, 'md')
else
redis:srem(DEVIL..hash, result.sender_user_id_)
...
local hash = 'bot:vipmems:'
if result.id_ then
if not redis:sismember(DEVIL..hash, result.id_) then
send(msg.chat_id_, msg.id_, 1,'👨‍✈️┊ العضو » ([@'..ap[2]..'])\n⚠️┊ تم تنزيله مميز عام سابقا\n‏', 1, 'md')
else
redis:srem(DEVIL..hash, result.id_)
...
if user_info_ then
local hash = 'bot:vipmems:'
if not redis:sismember(DEVIL..hash, ap[2]) then
send(msg.chat_id_, msg.id_, 1,'👨‍✈️┊ العضو » (['..UserDEVIL..'])\n⚠️┊ تم تنزيله مميز عتم سابقا\n‏', 1, 'md')
else
redis:srem(DEVIL..hash, ap[2])
send(msg.chat_id_, msg.id_, 1,'👨‍✈️┊ العضو » (['..UserDEVIL..'])\n⚠️┊ تم حذفه من مميزين العام \n✓‏', 1, 'md')
end end
end
-----------------------------promote_by_reply-------------------------------------------------------
...
function promote_by_username(extra, result, success)
if result.id_ then
if redis:sismember(DEVIL..'bot:vipmem:'..msg.chat_id_, result.id_) then
texts = '👨‍✈️┊ العضو » ([@'..ap[2]..'])\n⚠️┊ مرفوع مميز سابقا\n✓'
else
redis:sadd(DEVIL..'bot:vipmem:'..msg.chat_id_, result.id_)
texts = '👨‍✈️┊ العضو » ([@'..ap[2]..'])\n⚠️┊ تم رفعه عضو مميز\n✓'
end
...
local UserDEVIL = user_info_
if user_info_ then
if redis:sismember(DEVIL..'bot:vipmem:'..msg.chat_id_, ap[2]) then
texts = '👨‍✈️┊ العضو » (['..UserDEVIL..'])\n⚠️┊ مرفوع مميز سابقا\n✓'
else
redis:sadd(DEVIL..'bot:vipmem:'..msg.chat_id_, ap[2])
texts = '👨‍✈️┊ العضو » (['..UserDEVIL..'])\n⚠️┊ تم رفعه عضو مميز\n✓'
end end
send(msg.chat_id_, msg.id_, 1, texts, 1, 'md')
end
...
HTTPS.request('https://api.telegram.org/bot'..DEVIL_TOKEN..'/restrictChatMember?chat_id=' ..msg.chat_id_.. '&user_id=' ..result.sender_user_id_..'')
redis:sadd(DEVIL..hash, result.sender_user_id_)
send(msg.chat_id_, msg.id_, 1, '👨‍✈️┊ العضو » (['..UserDEVIL..'])\n⚠️┊ تم تقييده بنجاح \n✓‏', 1, 'md')
end
end
...
HTTPS.request('https://api.telegram.org/bot'..DEVIL_TOKEN..'/restrictChatMember?chat_id=' ..msg.chat_id_.. '&user_id=' ..result.id_..'')
redis:sadd(DEVIL..hash, result.id_)
send(msg.chat_id_, msg.id_, 1, '👨‍✈️┊ العضو » ([@'..ap[2]..'])\n⚠️┊ تم تقييده بنجاح \n✓‏', 1, 'md')
end
end
...
HTTPS.request('https://api.telegram.org/bot'..DEVIL_TOKEN..'/restrictChatMember?chat_id=' ..msg.chat_id_.. '&user_id=' ..ap[2]..'')
redis:sadd(DEVIL..hash, ap[2])
send(msg.chat_id_, msg.id_, 1, '👨‍✈️┊ العضو » (['..UserDEVIL..'])\n⚠️┊ تم تقييده بنجاح \n✓‏', 1, 'md')
end
end
...
HTTPS.request('https://api.telegram.org/bot'..DEVIL_TOKEN..'/restrictChatMember?chat_id=' ..msg.chat_id_.. '&user_id=' ..result.sender_user_id_..'&can_send_messages=True&can_send_media_messages=True&can_send_other_messages=True&can_add_web_page_previews=True')
redis:srem(DEVIL..hash, result.sender_user_id_)
...
HTTPS.request('https://api.telegram.org/bot'..DEVIL_TOKEN..'/restrictChatMember?chat_id=' ..msg.chat_id_.. '&user_id=' ..result.id_..'&can_send_messages=True&can_send_media_messages=True&can_send_other_messages=True&can_add_web_page_previews=True')
redis:srem(DEVIL..hash, result.id_)
send(msg.chat_id_, msg.id_, 1, '👨‍✈️┊ العضو » ([@'..ap[2]..'])\n⚠️┊ تم الغاء تقييده بنجاح \n✓‏', 1, 'md')
end
end
...
HTTPS.request('https://api.telegram.org/bot'..DEVIL_TOKEN..'/restrictChatMember?chat_id=' ..msg.chat_id_.. '&user_id=' ..ap[2]..'&can_send_messages=True&can_send_media_messages=True&can_send_other_messages=True&can_add_web_page_previews=True')
redis:srem(DEVIL..hash, ap[2])
send(msg.chat_id_, msg.id_, 1, '👨‍✈️┊ العضو » (['..UserDEVIL..'])\n⚠️┊ تم الغاء تقييده بنجاح \n✓‏', 1, 'md')
end
end
...
if result.id_ then
redis:sadd(DEVIL..'bot:admins:', result.id_)
texts = '👨‍✈️┊ العضو » ([@'..ap[2]..'])\n⚠️┊ تم اضافه ادمن في البوت \n✓‏'
else
...
local UserDEVIL = user_info_
if user_info_ then
local k1 = not redis:sismember(DEVIL..'bot:owners:'..msg.chat_id_, result.sender_user_id_)
local k2 = not redis:sismember(DEVIL..'bot:momod:'..msg.chat_id_, result.sender_user_id_)
local k3 = not redis:sismember(DEVIL..'bot:vipmem:'..msg.chat_id_, result.sender_user_id_)
if k1 and k2 and k3 then
send(msg.chat_id_, msg.id_, 1, '👨‍✈️┊ العضو » (['..UserDEVIL..'])\n⚠️┊ لا توجد لديه رتبه\n✓‏', 1, 'md')
else
redis:srem(DEVIL..'bot:owners:'..msg.chat_id_, result.sender_user_id_)
redis:srem(DEVIL..'bot:momod:'..msg.chat_id_, result.sender_user_id_)
redis:srem(DEVIL..'bot:vipmem:'..msg.chat_id_, result.sender_user_id_)
...
function delallrtb(extra, result, success)
if result.id_ then
local k1 = not redis:sismember(DEVIL..'bot:owners:'..msg.chat_id_, result.id_)
local k2 = not redis:sismember(DEVIL..'bot:momod:'..msg.chat_id_, result.id_)
local k3 = not redis:sismember(DEVIL..'bot:vipmem:'..msg.chat_id_, result.id_)
if k1 and k2 and k3 then
send(msg.chat_id_, msg.id_, 1, '👨‍✈️┊ العضو » ([@'..ap[2]..'])\n⚠️┊ لا توجد لديه رتبه\n✓‏', 1, 'md')
else
redis:srem(DEVIL..'bot:owners:'..msg.chat_id_, result.id_)
redis:srem(DEVIL..'bot:momod:'..msg.chat_id_, result.id_)
redis:srem(DEVIL..'bot:vipmem:'..msg.chat_id_, result.id_)
send(msg.chat_id_, msg.id_, 1, '👨‍✈️┊ العضو » ([@'..ap[2]..'])\n⚠️┊ تم حذف كل الرتب عنه \n✓‏', 1, 'md')
end
end
end
...
local UserDEVIL = user_info_
if user_info_ then
local k1 = not redis:sismember(DEVIL..'bot:owners:'..msg.chat_id_, ap[2])
local k2 = not redis:sismember(DEVIL..'bot:momod:'..msg.chat_id_, ap[2])
local k3 = not redis:sismember(DEVIL..'bot:vipmem:'..msg.chat_id_, ap[2])
if k1 and k2 and k3 then
send(msg.chat_id_, msg.id_, 1, '👨‍✈️┊ العضو » (['..UserDEVIL..'])\n⚠️┊ لا توجد لديه رتبه\n✓‏', 1, 'md')
else
redis:srem(DEVIL..'bot:owners:'..msg.chat_id_, ap[2])
redis:srem(DEVIL..'bot:momod:'..msg.chat_id_, ap[2])
redis:srem(DEVIL..'bot:vipmem:'..msg.chat_id_, ap[2])
send(msg.chat_id_, msg.id_, 1, '👨‍✈️┊ العضو » (['..UserDEVIL..'])\n⚠️┊ تم حذف كل الرتب عنه \n✓‏', 1, 'md')
end
end
end
...
local UserDEVIL = user_info_
if user_info_ then
local k1 = not redis:sismember(DEVIL..'bot:keed:'..msg.chat_id_, result.sender_user_id_)
local k2 = not redis:sismember(DEVIL..'bot:muted:'..msg.chat_id_, result.sender_user_id_)
local k3 = not redis:sismember(DEVIL..'bot:banned:'..msg.chat_id_, result.sender_user_id_)
if k1 and k2 and k3 then
send(msg.chat_id_, msg.id_, 1, '👨‍✈️┊ العضو » (['..UserDEVIL..'])\n⚠️┊ لا توجد لديه قيـــــــود\n✓‏', 1, 'md')
else
HTTPS.request('https://api.telegram.org/bot'..DEVIL_TOKEN..'/restrictChatMember?chat_id=' ..msg.chat_id_.. '&user_id=' ..result.sender_user_id_..'&can_send_messages=True&can_send_media_messages=True&can_send_other_messages=True&can_add_web_page_previews=True')
redis:srem(DEVIL..'bot:keed:'..msg.chat_id_, result.sender_user_id_)
redis:srem(DEVIL..'bot:muted:'..msg.chat_id_, result.sender_user_id_)
redis:srem(DEVIL..'bot:banned:'..msg.chat_id_, result.sender_user_id_)
send(msg.chat_id_, msg.id_, 1, '👨‍✈️┊ العضو » (['..UserDEVIL..'])\n⚠️┊ تم رفع القيود عنه \n✓‏', 1, 'md')
end
end
end
...
function delallrtb(extra, result, success)
if result.id_ then
local k1 = not redis:sismember(DEVIL..'bot:keed:'..msg.chat_id_, result.id_)
local k2 = not redis:sismember(DEVIL..'bot:muted:'..msg.chat_id_, result.id_)
local k3 = not redis:sismember(DEVIL..'bot:banned:'..msg.chat_id_, result.id_)
if k1 and k2 and k3 then
send(msg.chat_id_, msg.id_, 1, '👨‍✈️┊ العضو » ([@'..ap[2]..'])\n⚠️┊ لا توجد لديه قيـــــــود\n✓‏', 1, 'md')
else
HTTPS.request('https://api.telegram.org/bot'..DEVIL_TOKEN..'/restrictChatMember?chat_id=' ..msg.chat_id_.. '&user_id=' ..result.id_..'&can_send_messages=True&can_send_media_messages=True&can_send_other_messages=True&can_add_web_page_previews=True')
redis:srem(DEVIL..'bot:keed:'..msg.chat_id_, result.id_)
redis:srem(DEVIL..'bot:muted:'..msg.chat_id_, result.id_)
redis:srem(DEVIL..'bot:banned:'..msg.chat_id_, result.id_)
send(msg.chat_id_, msg.id_, 1, '👨‍✈️┊ العضو » ([@'..ap[2]..'])\n⚠️┊ تم رفع القيود عنه \n✓‏', 1, 'md')
end
end
end
...
local UserDEVIL = user_info_
if user_info_ then
local k1 = not redis:sismember(DEVIL..'bot:keed:'..msg.chat_id_, ap[2])
local k2 = not redis:sismember(DEVIL..'bot:muted:'..msg.chat_id_, ap[2])
local k3 = not redis:sismember(DEVIL..'bot:banned:'..msg.chat_id_, ap[2])
if k1 and k2 and k3 then
send(msg.chat_id_, msg.id_, 1, '👨‍✈️┊ العضو » (['..UserDEVIL..'])\n⚠️┊ لا توجد لديه قيـــــــود\n✓‏', 1, 'md')
else
HTTPS.request('https://api.telegram.org/bot'..DEVIL_TOKEN..'/restrictChatMember?chat_id=' ..msg.chat_id_.. '&user_id=' ..ap[2]..'&can_send_messages=True&can_send_media_messages=True&can_send_other_messages=True&can_add_web_page_previews=True')
redis:srem(DEVIL..'bot:keed:'..msg.chat_id_, ap[2])
redis:srem(DEVIL..'bot:muted:'..msg.chat_id_, ap[2])
redis:srem(DEVIL..'bot:banned:'..msg.chat_id_, ap[2])
send(msg.chat_id_, msg.id_, 1, '👨‍✈️┊ العضو » (['..UserDEVIL..'])\n⚠️┊ تم رفع القيود عنه \n✓‏', 1, 'md')
end
end
end
...
if redis:get(DEVIL.."lock_GEM"..msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ اللعبه معطله\n ‏ ", 1, "md")
return false end
local user_info_ = redis:get(DEVIL.."user:Name" .. msg.sender_user_id_)
local UserDEVIL = user_info_
if user_info_ then
redis:set(DEVIL..'kk1'..msg.sender_user_id_..''..msg.chat_id_..'','kk')
send(msg.chat_id_, 0, 1, '👨‍✈️» اهلا ['..UserDEVIL..'] \n™️» في لعبه السرعه\n⚠️» ارسل ( بدء اللعبه ) للعب\n✓',1, 'md')
return false end end
if text == 'بدء اللعبه'  and is_momod(msg.sender_user_id_, msg.chat_id_) and redis:get(DEVIL..'kk1'..msg.sender_user_id_..''..msg.chat_id_..'') then
local keep1 = {'س م ى و','ى-س-م-و'}
send(msg.chat_id_, 0, 1, '• رتب الكلمه  التاليه \n*('..keep1[math.random(#keep1)]..')*',1, 'md')
redis:set(DEVIL..'kk11'..msg.chat_id_..'','kkk')
return false end
if text then
local DEVIL1 = redis:get(DEVIL..'kk11'..msg.chat_id_..'')
if DEVIL1 == 'kkk' then
if text == 'موسى' then
redis:incr(DEVIL..'incr_msg'..msg.sender_user_id_..''..msg.chat_id_..'')
send(msg.chat_id_, msg.id_, 1, '• اجابتك صحيحه 👏🏻 ',1, 'md')
redis:del(DEVIL..'kk11'..msg.chat_id_..'')
sleep(1.5)
local ooo = {'ا-ل-ي-ي-ب','ي ي ا ل ب'}
send(msg.chat_id_, 0, 1, '• رتب الكلمه  التاليه \n*('..ooo[math.random(#ooo)]..')*',1, 'md')
redis:set(DEVIL..'kk111'..msg.chat_id_..'','kkkk')
 return false end end end
if text then
local DEVIL1 = redis:get(DEVIL..'kk111'..msg.chat_id_..'')
if DEVIL1 == 'kkkk' then
if text == 'ليبيا' then
redis:incr(DEVIL..'incr_msg'..msg.sender_user_id_..''..msg.chat_id_..'')
send(msg.chat_id_, msg.id_, 1, '• اجابتك صحيحه 👏🏻 ',1, 'md')
redis:del(DEVIL..'kk111'..msg.chat_id_..'')
sleep(1.5) 
local keep = {'ر ط ا ي ه','ا-ي-ط-ر-ه'}
send(msg.chat_id_, 0, 1, '• رتب الكلمه  التاليه \n*('..keep[math.random(#keep)]..')*',1, 'md')
redis:set(DEVIL..'kk1111'..msg.chat_id_..'','mm')
 return false end end end
if text then
local DEVIL1 = redis:get(DEVIL..'kk1111'..msg.chat_id_..'')
if DEVIL1 == 'mm' then
if text == 'طياره' then
redis:incr(DEVIL..'incr_msg'..msg.sender_user_id_..''..msg.chat_id_..'')
send(msg.chat_id_, msg.id_, 1, '• اجابتك صحيحه 👏🏻 ',1, 'md')
redis:del(DEVIL..'kk1111'..msg.chat_id_..'')
sleep(1.5)
local keep77 = {'😛😛😛😝😛😛😛','😜😝😜😜😜😜😜'}
send(msg.chat_id_, 0, 1, '•ارسل الاسمايل المختلف \n*('..keep77[math.random(#keep77)]..')*',1, 'md')
redis:set(DEVIL..'w1'..msg.chat_id_..'','q1')
 return false end end end
if text then
local DEVIL1 = redis:get(DEVIL..'w1'..msg.chat_id_..'')
if DEVIL1 == 'q1' then
if text == '😝' then
redis:incr(DEVIL..'incr_msg'..msg.sender_user_id_..''..msg.chat_id_..'')
send(msg.chat_id_, msg.id_, 1, '• اجابتك صحيحه 👏🏻 ',1, 'md')
redis:del(DEVIL..'w1'..msg.chat_id_..'')
sleep(1.5)
local keep2 = {'ش-ر-ط-ي','ش,ط,ر,ي'}
send(msg.chat_id_, 0, 1, '• رتب الكلمه  التاليه \n*('..keep2[math.random(#keep2)]..')*',1, 'md')
redis:set(DEVIL..'w2'..msg.chat_id_..'','q2')
 return false end end end
if text then
local DEVIL1 = redis:get(DEVIL..'w2'..msg.chat_id_..'')
if DEVIL1 == 'q2' then
if text == 'شرطي' then
redis:incr(DEVIL..'incr_msg'..msg.sender_user_id_..''..msg.chat_id_..'')
send(msg.chat_id_, msg.id_, 1, '• اجابتك صحيحه 👏🏻 ',1, 'md')
redis:del(DEVIL..'w2'..msg.chat_id_..'')
sleep(1.5)
local keep3 = {'😃😃😄😃😃😃'}
send(msg.chat_id_, 0, 1, '•ارسل الاسمايل المختلف \n*('..keep3[math.random(#keep3)]..')*',1, 'md')
redis:set(DEVIL..'w9'..msg.chat_id_..'','q9')
 return false end end end
if text then
local DEVIL1 = redis:get(DEVIL..'w9'..msg.chat_id_..'')
if DEVIL1 == 'q9' then
if text == '😄' then
redis:incr(DEVIL..'incr_msg'..msg.sender_user_id_..''..msg.chat_id_..'')
send(msg.chat_id_, msg.id_, 1, '• اجابتك صحيحه 👏🏻 ',1, 'md')
redis:del(DEVIL..'w9'..msg.chat_id_..'')
sleep(1.5)
local keep4 = {'ر ك و ا ي','ر ا ي ك و'}
send(msg.chat_id_, 0, 1, '• رتب الكلمه  التاليه \n*('..keep4[math.random(#keep4)]..')*',1, 'md')
redis:set(DEVIL..'w4'..msg.chat_id_..'','q4')
 return false end end end
if text then
local DEVIL1 = redis:get(DEVIL..'w4'..msg.chat_id_..'')
if DEVIL1 == 'q4' then
if text == 'كوريا' then
redis:incr(DEVIL..'incr_msg'..msg.sender_user_id_..''..msg.chat_id_..'')
send(msg.chat_id_, msg.id_, 1, '• اجابتك صحيحه 👏🏻 ',1, 'md')
redis:del(DEVIL..'w4'..msg.chat_id_..'')
sleep(1.5)
local keep5 = {'ك - م - ا - ل','ل ك ا م'}
send(msg.chat_id_, 0, 1, '• رتب الكلمه  التاليه \n*('..keep5[math.random(#keep5)]..')*',1, 'md')
redis:set(DEVIL..'w5'..msg.chat_id_..'','q5')
return false end end end
if text then
local DEVIL1 = redis:get(DEVIL..'w5'..msg.chat_id_..'')
if DEVIL1 == 'q5' then
if text == 'ملاك' then
redis:incr(DEVIL..'incr_msg'..msg.sender_user_id_..''..msg.chat_id_..'')
send(msg.chat_id_, msg.id_, 1, '• اجابتك صحيحه 👏🏻 ',1, 'md')
redis:del(DEVIL..'w5'..msg.chat_id_..'')
sleep(1.5)
send(msg.chat_id_, 0, 1, '• رتب الكلمه  التاليه \n(ك , ش , ب , ا)',1, 'md')
redis:set(DEVIL..'w6'..msg.chat_id_..'','q6')
 return false end end end
if text then
local DEVIL1 = redis:get(DEVIL..'w6'..msg.chat_id_..'')
if DEVIL1 == 'q6' then
if text == 'شباك' then
redis:incr(DEVIL..'incr_msg'..msg.sender_user_id_..''..msg.chat_id_..'')
send(msg.chat_id_, msg.id_, 1, '• اجابتك صحيحه 👏🏻 ',1, 'md')
redis:del(DEVIL..'w6'..msg.chat_id_..'')
sleep(1.5)
local keep6 = {'ل-ج-م-ي','ل ي م ج'}
send(msg.chat_id_, 0, 1, '• رتب الكلمه  التاليه \n*('..keep6[math.random(#keep6)]..')*',1, 'md')
redis:set(DEVIL..'w7'..msg.chat_id_..'','q7')
return false end end end
if text then
local DEVIL1 = redis:get(DEVIL..'w7'..msg.chat_id_..'')
if DEVIL1 == 'q7' then
if text == 'جميل' then
redis:incr(DEVIL..'incr_msg'..msg.sender_user_id_..''..msg.chat_id_..'')
send(msg.chat_id_, msg.id_, 1, '• اجابتك صحيحه 👏🏻 ',1, 'md')
redis:del(DEVIL..'w7'..msg.chat_id_..'')
sleep(1.5)
local keep7 = {'و ك ه ه','ه ك ه و'}
send(msg.chat_id_, 0, 1, '• رتب الكلمه  التاليه \n*('..keep7[math.random(#keep7)]..')*',1, 'md')
redis:set(DEVIL..'w8'..msg.chat_id_..'','q8')
return false end end end
if text then
local DEVIL1 = redis:get(DEVIL..'w8'..msg.chat_id_..'')
if DEVIL1 == 'q8' then
if text == 'كهوه' then
redis:incr(DEVIL..'incr_msg'..msg.sender_user_id_..''..msg.chat_id_..'')
send(msg.chat_id_, msg.id_, 1, '• اجابتك صحيحه 👏🏻 ',1, 'md')
redis:del(DEVIL..'w8'..msg.chat_id_..'')
sleep(1.5)
send(msg.chat_id_, 0, 1, '• رتب الكلمه  التاليه \n( ن ،ص، و، ك، د)',1, 'md')
redis:set(DEVIL..'w9o'..msg.chat_id_..'','q9o')
 return false end end end
if text then
local DEVIL1 = redis:get(DEVIL..'w9o'..msg.chat_id_..'')
if DEVIL1 == 'q9o' then
if text == 'صندوك' then
redis:incr(DEVIL..'incr_msg'..msg.sender_user_id_..''..msg.chat_id_..'')
send(msg.chat_id_, msg.id_, 1, '• اجابتك صحيحه 👏🏻 ',1, 'md')
redis:del(DEVIL..'w9o'..msg.chat_id_..'')
sleep(1.5)
local keep8 = {'ض ر م ي','م ض ر ي'}
send(msg.chat_id_, 0, 1, '• رتب الكلمه  التاليه \n*('..keep8[math.random(#keep8)]..')*',1, 'md')
redis:set(DEVIL..'w00'..msg.chat_id_..'','q00')
 return false end end end
if text then
local DEVIL1 = redis:get(DEVIL..'w00'..msg.chat_id_..'')
if DEVIL1 == 'q00' then
if text == 'مريض' then
redis:incr(DEVIL..'incr_msg'..msg.sender_user_id_..''..msg.chat_id_..'')
send(msg.chat_id_, msg.id_, 1, '• اجابتك صحيحه 👏🏻 ',1, 'md')
redis:del(DEVIL..'w00'..msg.chat_id_..'')
sleep(1.5)
local keep9 = {'ي- ط -ر -ق -ن -ب -ا',' ب ي ق ا ط ر ا ن','ب,ي,ا,,ق,ر,ن,ط'}
send(msg.chat_id_, 0, 1, '• رتب الكلمه  التاليه \n*('..keep9[math.random(#keep9)]..')*',1, 'md')
redis:set(DEVIL..'a15'..msg.chat_id_..'','s15')
 return false end end end
if text then
local DEVIL1 = redis:get(DEVIL..'a15'..msg.chat_id_..'')
if DEVIL1 == 's15' then
if text == 'قرنابيط' then
redis:incr(DEVIL..'incr_msg'..msg.sender_user_id_..''..msg.chat_id_..'')
send(msg.chat_id_, msg.id_, 1, '• اجابتك صحيحه 👏🏻 ',1, 'md')
redis:del(DEVIL..'a15'..msg.chat_id_..'')
sleep(1.5)
local keep00 = {'😔😔😔😔😔😔😔☺️😔😔😔😔😔'}
send(msg.chat_id_, 0, 1, '•ارسل الاسمايل المختلف \n*('..keep00[math.random(#keep00)]..')*',1, 'md')
redis:set(DEVIL..'a26'..msg.chat_id_..'','s26')
return false end end end
if text then
local DEVIL1 = redis:get(DEVIL..'a26'..msg.chat_id_..'')
if DEVIL1 == 's26' then
if text == '☺️' then
redis:incr(DEVIL..'incr_msg'..msg.sender_user_id_..''..msg.chat_id_..'')
send(msg.chat_id_, msg.id_, 1, '• اجابتك صحيحه 👏🏻 ',1, 'md')
redis:del(DEVIL..'a26'..msg.chat_id_..'')
sleep(1.5)
local keepp = {'ط - م - ي','ي ,م ,ط'}
send(msg.chat_id_, 0, 1, '• رتب الكلمه  التاليه \n*('..keepp[math.random(#keepp)]..')*',1, 'md')
redis:set(DEVIL..'a99'..msg.chat_id_..'','s99')
 return false end end end
if text then
local DEVIL1 = redis:get(DEVIL..'a99'..msg.chat_id_..'')
if DEVIL1 == 's99' then
if text == 'مطي' then
redis:incr(DEVIL..'incr_msg'..msg.sender_user_id_..''..msg.chat_id_..'')
send(msg.chat_id_, msg.id_, 1, '• اجابتك صحيحه 👏🏻 ',1, 'md')
redis:del(DEVIL..'a99'..msg.chat_id_..'')
return false end end end
-----------------------------------------------------------------------------
if text == 'نقاطي' then
send(msg.chat_id_, msg.id_, 1, '💬┊ عدد نقاطك » *('..tonumber(redis:get(DEVIL..'incr_msg'..msg.sender_user_id_..''..msg.chat_id_..'') or 0 )..')*',1, 'md')
return false end
if text == 'بيع نقاطي' then
local user_info_ = redis:get(DEVIL..'user:Name' .. msg.sender_user_id_)
local UserDEVIL = user_info_
if user_info_ then
local numkep = tonumber(redis:get(DEVIL..'incr_msg'..msg.sender_user_id_..''..msg.chat_id_..'') or 0 )
if numkep == 0 then
...
redis:set(DEVIL..'karrar1'..msg.sender_user_id_..''..msg.chat_id_..'','karrar2')
send(msg.chat_id_, msg.id_, 1, keep,1, 'md')
return false end end end
if text then
local DEVIL1 = redis:get(DEVIL..'karrar1'..msg.sender_user_id_..''..msg.chat_id_..'')
if DEVIL1 == 'karrar2' then
if text == '1' then
if redis:sismember(DEVIL..'bot:vipmem:'..msg.chat_id_, msg.sender_user_id_) then
send(msg.chat_id_, msg.id_, 1,'⚠️┊ عذرا انت مرفوع مميز سابقا\n✓', 1, 'md')
redis:del(DEVIL..'karrar1'..msg.sender_user_id_..''..msg.chat_id_..'')
else
local numkep = tonumber(redis:get(DEVIL..'incr_msg'..msg.sender_user_id_..''..msg.chat_id_..'') or 0 )
if numkep < tonumber(redis:get(DEVIL.."DEVIL_OO0" .. msg.chat_id_) or 100 )  then
send(msg.chat_id_, msg.id_, 1, '®️┊عذرا نقاطك اقل من '..tonumber(redis:get(DEVIL.."DEVIL_OO0" .. msg.chat_id_) or 100 )..'\n💬┊لا تستطيع تحويلهم لعضو مميز\n✓',1, 'md')
redis:del(DEVIL..'karrar1'..msg.sender_user_id_..''..msg.chat_id_..'')
return false end
if numkep > tonumber(redis:get(DEVIL.."DEVIL_OO0" .. msg.chat_id_) or 100 )  then
send(msg.chat_id_, msg.id_, 1, '🎰┊مبروك عزيزي\n🎧┊تم رفعك عضو مميز\n ✓',1, 'md')
redis:sadd(DEVIL..'bot:vipmem:'..msg.chat_id_, msg.sender_user_id_)
redis:del(DEVIL..'incr_msg'..msg.sender_user_id_..''..msg.chat_id_..'')
redis:del(DEVIL..'karrar1'..msg.sender_user_id_..''..msg.chat_id_..'')
...
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmDEVIL(msg).."\n🎟┊ تم تفعيل امر اطردني\n ✓ ", 1, 'md')
redis:set(DEVIL.."lock_kickme"..msg.chat_id_, true)
return false 
end
if text == 'تعطيل اطردني' and is_owner(msg.sender_user_id_, msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmDEVIL(msg).."\n🎟┊ تم تعطيل امر اطردني\n ✓ ", 1, 'md')
redis:del(DEVIL.."lock_kickme"..msg.chat_id_)
return false 
end
-------------------add ch --------------------------
...
redis:setex(DEVIL..'Kpch'..msg.sender_user_id_,300,true)
...
if not redis:get(DEVIL..'Kpch1') then
send(msg.chat_id_, msg.id_, 1, '™️┊ لم يتم وضع القناة\n📌┊ لتعين  القناة ارسل .....\n⚠️┊ `تعين قناة الاشتراك `\n➖', 1, 'md')
return false 
end
if redis:get(DEVIL..'Kpch1') then
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmDEVIL(msg).."\n🎟┊ تم تفعيل الاشتراك الاجباري\n ✓ ", 1, 'md')
redis:set(DEVIL.."Kpjoin1", true)
return false end end end
...
redis:del(DEVIL.."dellmssg"..msg.chat_id_)
end
----------------ADD DEL MSG------------------------------------------------------------
...
redis:set(DEVIL.."dellmssg"..msg.chat_id_, true)
end
----------add - rem bot free----------------------------------
...
local UserDEVIL = user_info_
if user_info_ then
local start = redis:get(DEVIL.."startbot")
if start then
keep = [[
[]]..start..[[]
]]
...
💠┊مرحبا انا بوت اسمي *(]]..(redis:get(DEVIL..'DEVILnams') or 'DEVIL')..[[)🗼*
...
🔱┊*مطور البوت* » ( []] .. UserDEVIL .. [[] )‏
‏
‏]]
send(msg.chat_id_, msg.id_, 1, DEVIL, 1, 'md')
end end end
-----------------------get start------------------------------------------------------------------------
...
local Kpch2 = redis:get(DEVIL.."Kpch1")
if Kpch2 then
...
text = "🌀┊  ادمنيه البوت :\n*≖≖≖≖≖≖≖≖≖≖≖≖*\n"
for k, v in pairs(list) do
if redis:get(DEVIL.."SudoNumberGp" .. v) then
gps = tonumber(redis:get(DEVIL.."SudoNumberGp" .. v))
else
gps = 0
end
local user_info = redis:get(DEVIL.."user:Name" .. v)
...
local time = a[2] * day
redis:setex(DEVIL.."bot:charge:"..msg.chat_id_,time,true)
redis:set(DEVIL.."bot:enable:"..msg.chat_id_,true)
end
------------------charge-----------------------------------------------------------------------------
if text:match("^فحص الشحن") and is_momod(msg.sender_user_id_, msg.chat_id_) then
local ex = redis:ttl(DEVIL.."bot:charge:"..msg.chat_id_)
if ex == -1 then
...
local ex = redis:ttl(DEVIL.."bot:charge:"..txt[2])
if ex == -1 then
...
if not redis:get(DEVIL.."bot:free") and not is_sudo(msg) then
send(msg.chat_id_, msg.id_, 1, '🚫┊* للمطوريـــن فقـــط* ❗️', 1, 'md')
return false end
local function promote_admin(extra, result, success)
local admins = result.members_
for i=0 , #admins do
redis:sadd(DEVIL..'bot:momod:'..msg.chat_id_,admins[i].user_id_)
if result.members_[i].status_.ID == "ChatMemberStatusCreator" then
owner_id = admins[i].user_id_
redis:sadd(DEVIL.."bot:owners:"..msg.chat_id_,owner_id)
end
end
end
getChannelMembers(msg.chat_id_, 0, 'Administrators', 200, promote_admin)
redis:set(DEVIL.."test:group"..msg.chat_id_,'krar')
if redis:get(DEVIL.."bot:enable:" .. msg.chat_id_) then
...
if not redis:get(DEVIL.."bot:group:link"..msg.chat_id_) then
local getlink = 'https://api.telegram.org/bot'..DEVIL_TOKEN..'/exportChatInviteLink?chat_id='..msg.chat_id_
local req = https.request(getlink)
local link = KPJS:decode(req)
if link.ok == true then 
redis:set(DEVIL.."bot:group:link"..msg.chat_id_,link.result)
end
end
local lik_1 = redis:get(DEVIL.."bot:group:link"..msg.chat_id_)
if lik_1 then
link = lik_1 
else
link = link.result
end
send(Kp_Owner,0, 1,"- *تم اضافه مجموعه* »\nﮧ┉┉┉┉┉┉┉┉┉\n‏🔱┊ المدير ≈ [" .. owner .. "]\n🔰┊ ["..title_name(msg.chat_id_).."]("..(link or "t.me/DEVIL_ch")..")\n🚫┊ *ايدي المجموعه* »\n📉┊ﮧ "..msg.chat_id_.."\n*«معلومات عن المطور»*\nﮧ┉┉┉┉┉┉┉┉┉\n🌀┊ ايديه ≈ (" .. msg.sender_user_id_ .. ")\n🚫┊ اسمه ≈ " ..result.first_name_.. "\n️⚠️┊ معرفه ≈ [" .. username .. "]\n✓", 1, "md")
redis:set(DEVIL.."bot:enable:" .. msg.chat_id_, true)
redis:setex(DEVIL.."bot:charge:" .. msg.chat_id_, 9999 * day, true)
redis:sadd(DEVIL.."sudo:data:" .. msg.sender_user_id_, msg.chat_id_)
end
tdcli_function ({
ID = "GetChannelFull",
...
redis:set(DEVIL.."bot:charge:"..txt[2],true)
...
local txt = { string.match(text, "^(تفعيل)$")}
redis:del(DEVIL..'lock:add'..msg.chat_id_)
local function promote_admin(extra, result, success)
local admins = result.members_
for i=0 , #admins do
redis:sadd(DEVIL..'bot:momod:'..msg.chat_id_,admins[i].user_id_)
...
redis:set(DEVIL..'lock:add'..msg.chat_id_,true)
if not redis:get(DEVIL.."bot:enable:" .. msg.chat_id_) then
...
redis:del(DEVIL.."bot:enable:" .. msg.chat_id_)
redis:srem(DEVIL.."bot:groups", msg.chat_id_)
redis:del(DEVIL.."test:group"..msg.chat_id_)
redis:srem(DEVIL.."sudo:data:" .. msg.sender_user_id_, msg.chat_id_)
...
send(v,0, 1,"- *تم تعطيل مجموعه* »\nﮧ┉┉┉┉┉┉┉┉┉\n‏🔱┊ المدير ≈ " .. owner .. "\n🔰┊ الرابط ≈ [اضغـط هنـا](" .. (redis:get(DEVIL.."bot:group:link"..msg.chat_id_) or "https://t.me/DEVIL_ch") .. ")\n🔅┊* اسم المجموعه* »\n👨🏼┊ﮧ "..title_name(msg.chat_id_).."\n🚫┊ *ايدي المجموعه* »\n📉┊ﮧ "..msg.chat_id_.."\n*«معلومات عن المطور»*\nﮧ┉┉┉┉┉┉┉┉┉\n🌀┊ ايديه ≈ (" .. msg.sender_user_id_ .. ")\n🚫┊ اسمه ≈ " .. fname .. " " .. lname .. "\n️⚠️┊ معرفه ≈ [" .. username .. "]\n‏", 1, "md")
end
...
local gp = txt[3]
send(msg.chat_id_, msg.id_, 1, "🌀┊ تم تفعيل المجموعه \n(*"..txt[2].."*) يوم 🍃", 1, 'html')
redis:sadd(DEVIL..'sudo:data:'..sudo, gp)
end
-------------------REEM----------------------------------------------------------------------------
...
local send_to_bot_owner = function(extra, result)
redis:del(DEVIL.."bot:enable:" .. gp[2])
redis:del(DEVIL.."bot:charge:" .. gp[2])
local v = tonumber(Kp_Owner)
local fname = result.first_name_ or ""
local lname = result.last_name_ or ""
local username = "@" .. result.username_ or ""
...
redis:srem(DEVIL.."sudo:data:" .. msg.sender_user_id_, gp[2])
redis:srem(DEVIL.."bot:groups", gp[2])
end
getUser(msg.sender_user_id_, send_to_bot_owner)
end
else
...
local num_keep = (tonumber(redis:get(DEVIL.."incr_msg"..result.id_..""..msg.chat_id_.."") or 0 ))
local msgs = (tonumber(redis:get(DEVIL.."msgs:"..result.id_..":"..msg.chat_id_) or  0))
local Kpcontact = (tonumber(redis:get(DEVIL.."kpaddcon"..msg.chat_id_..":"..result.id_) or 0))
if result.id_ then
...
local UserDEVIL = user_info_
if user_info_ then
send(msg.chat_id_, msg.id_, 1, "•اهلا ~ ["..UserDEVIL.."]\n🎧» ايديك  (`"..msg.sender_user_id_.."`)\n✓", 1, "md")
return false end end
-------------------RETBA-------------------------
...
if not redis:get(DEVIL..'lock:add'..msg.chat_id_) then
local num_keep = tonumber(redis:get(DEVIL..'incr_msg'..msg.sender_user_id_..''..msg.chat_id_..'') or 0 )
local msgs = tonumber(redis:get(DEVIL.."msgs:"..msg.sender_user_id_..":"..msg.chat_id_))
local Kpcontact = (tonumber(redis:get(DEVIL.."kpaddcon"..msg.chat_id_..":"..msg.sender_user_id_) or 0))
...
if redis:get(DEVIL.."getidstatus" .. msg.chat_id_) == "Photo" then
...
if redis:get(DEVIL.."getidstatus" .. msg.chat_id_) == "Simple" then
send(msg.chat_id_, msg.id_, 1, kepper_info2, 1, "md")
end
if not redis:get(DEVIL.."getidstatus" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, kepper_info2, 1, "md")
 end 
 end
tdcli_function({
...
--------------------Kp concat add----------------------------------------
if text == 'جهاتي' then
send(msg.chat_id_, msg.id_, 1, '🔰┊ عدد جهاتك ≈ * '..(tonumber(redis:get(DEVIL..'kpaddcon'..msg.chat_id_..':'..msg.sender_user_id_) or 0))..' *',1,'md')
end
--------------------ID BY REPLY------------------------------------------
if text:match("^ايدي$") and msg.reply_to_message_id_ ~= 0 then
function iD_reP(extra, result, success)
local num_keep = (tonumber(redis:get(DEVIL.."incr_msg"..result.sender_user_id_..""..msg.chat_id_.."") or 0 ))
local msgs = (tonumber(redis:get(DEVIL.."msgs:"..result.sender_user_id_..":"..msg.chat_id_) or 0 ))
local Kpcontact = (tonumber(redis:get(DEVIL.."kpaddcon"..msg.chat_id_..":"..result.sender_user_id_) or 0))
if result.id_ then
if tonumber(result.sender_user_id_) == tonumber(Kp_Owner) then
DEVIL3 = "مطور الاساسـي 🌿"
elseif is_sudoid(result.sender_user_id_) then
DEVIL3 = "المطور 🐾"
elseif is_admin(result.sender_user_id_) then
DEVIL3 = "ادمن في البوت"
...
local DEVIL_info = "🎟┊ ايديه  » `" .. result.sender_user_id_ .. "`\n👤┊ جهاته  »   "..Kpcontact.."\n🔆┊ نقاطه  »   "..num_keep.."\n🌐┊ تفاعله »  " .. KP_TM_NM(msgs) .. "\n✉️┊ رسائله » " .. msgs .. "\n📌┊ موقعه » " .. DEVIL3 .. "\n‏┄┄┄┄┄┄┄┄┄┄┄┄"
send(msg.chat_id_, result.id_, 1, DEVIL_info, 1, "md")
end
...
redis:setex(DEVIL..'botts:namess'..msg.sender_user_id_,698,true)
...
if redis:get(DEVIL.."getpro:" .. msg.chat_id_) == "Active" then
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmDEVIL(msg).."\n🎟┊ تم تفعيل جلب الصور\n ✓ ", 1, 'md')
else
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmDEVIL(msg).."\n🎟┊ تم تفعيل جلب الصور\n ✓ ", 1, 'md')
redis:set(DEVIL.."getpro:" .. msg.chat_id_, "Active")
end end end
...
if redis:get(DEVIL.."getpro:" .. msg.chat_id_) == "Deactive" then
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmDEVIL(msg).."\n🎟┊ تم تعطيل جلب الصور\n ✓ ", 1, 'md')
else
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmDEVIL(msg).."\n🎟┊ تم تعطيل جلب الصور\n ✓ ", 1, 'md')
redis:set(DEVIL.."getpro:" .. msg.chat_id_, "Deactive")
end end end
---------------------------------------------------------------------
if text:match("^مسح الصوره") and is_momod(msg.sender_user_id_, msg.chat_id_)  then
https.request('https://api.telegram.org/bot'..DEVIL_TOKEN..'/deleteChatPhoto?chat_id='..msg.chat_id_)
...
if not is_momod(msg.sender_user_id_, msg.chat_id_) and redis:get(DEVIL.."getpro:" .. msg.chat_id_) == "Deactive" then
...
redis:set(DEVIL..'flood:time:'..msg.chat_id_,floodt[2])
...
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmDEVIL(msg).."\n🎟┊ تم وضع التكرار بالطرد\n ✓ ", 1, 'md')
redis:set(DEVIL.."floodstatus" .. msg.chat_id_, "Kicked")
end
end
if status[2] == "بالمسح" then
if redis:get(DEVIL.."floodstatus" .. msg.chat_id_) == "DelMsg" then
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmDEVIL(msg).."\n🎟┊ تم وضع التكرار بالمسح\n ✓ ", 1, 'md')
else
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmDEVIL(msg).."\n🎟┊ تم وضع التكرار بالمسح\n ✓ ", 1, 'md')
redis:set(DEVIL.."floodstatus" .. msg.chat_id_, "DelMsg")
end
end
end
-------------------SET warn-----------------------------------------
...
redis:set(DEVIL.."warn:max:" .. msg.chat_id_, warnmax[2])
end
end
---------------------warnstatus----------------------------------------------------------
...
if redis:get(DEVIL.."warnstatus" .. msg.chat_id_) == "Muteuser" then
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmDEVIL(msg).."\n🎟┊ تم وضع التحذير بالكتم\n ✓ ", 1, 'md')
else
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmDEVIL(msg).."\n🎟┊ تم وضع التحذير بالكتم\n ✓ ", 1, 'md')
redis:set(DEVIL.."warnstatus" .. msg.chat_id_, "Muteuser")
end   end
if status[2] == "بالطرد" then
if redis:get(DEVIL.."warnstatus" .. msg.chat_id_) == "Remove" then
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmDEVIL(msg).."\n🎟┊ تم وضع التحذير بالطرد\n ✓ ", 1, 'md')
else
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmDEVIL(msg).."\n🎟┊ تم وضع التحذير بالطرد\n ✓ ", 1, 'md')
redis:set(DEVIL.."warnstatus" .. msg.chat_id_, "Remove")
end end end
-----------------------getidstatus-----------------------------------------------------------------
...
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmDEVIL(msg).."\n🎟┊ تم تفعيل الخروج التلقائي\n ✓ ", 1, 'md')
redis:set(DEVIL.."autoleave", "On")
end end end
if is_sudo(msg) and text:match("^تعطيل (.*)$") then
local status = { string.match(text, "^(تعطيل) (.*)$")}
if status[2] == "الخروج التلقائي" then
if redis:get(DEVIL.."autoleave") == "Off" then
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmDEVIL(msg).."\n🎟┊ تم تعطيل الخروج التلقائي\n ✓ ", 1, 'md')
else
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmDEVIL(msg).."\n🎟┊ تم تعطيل الخروج التلقائي\n ✓ ", 1, 'md')
redis:set(DEVIL.."autoleave", "Off")
end   end  end
-----------------------------------------------------------
...
redis:setex(DEVIL.."bot:DEVIL_dev" .. msg.chat_id_ .. ":" .. msg.sender_user_id_, 370, true)
end end
...
local DEVIL_dev = redis:get(DEVIL.."DEVIL_dev")
if DEVIL_dev then
send(msg.chat_id_, msg.id_, 1, "["..DEVIL_dev.."]", 1, "md")
else
local karrar = [[
...
redis:setex(DEVIL.."bot:group:link" .. msg.chat_id_ .. ":" .. msg.sender_user_id_, 1200, true)
end
if is_momod(msg.sender_user_id_, msg.chat_id_) and idf:match("-100(%d+)") and (text:match("^حذف الرابط$"))  then
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmDEVIL(msg).."\n🎟┊ تم حذف الرابط بنجاح\n ✓ ", 1, 'md')
redis:del(DEVIL.."bot:group:link" .. msg.chat_id_)
end
if text:match("^الرابط$") then
local link = redis:get(DEVIL.."bot:group:link" .. msg.chat_id_)
if link then
...
redis:setex(DEVIL.."Filtering:" .. msg.sender_user_id_, 80, msg.chat_id_)
end
----------------------------------------------------------------------------------
...
local hash = "bot:filters:" .. msg.chat_id_
if not redis:hget(DEVIL..hash, name) then
send(msg.chat_id_, msg.id_, 1, "🌀┊  الكلمه〖  " .. name .. "  〗\n👥┊  تم الغاء منعها📍", 1, "md")
redis:hdel(DEVIL..hash, name)
end end
-------------------USERS----------------------------------------------------
if text == 'المشتركين' or text == 'عدد المشتركين' and is_KP(msg) then
local users = redis:scard(DEVIL.."bot:userss")
local botnamess = redis:get(DEVIL.."DEVILnams") or "DEVIL"
...
local get_me = function(extra, result)
local num = (redis:scard(DEVIL.."bot:groups"))
local list = redis:smembers(DEVIL.."bot:groups")
local text = "~ All_Groups_Bots \n\n"
for k,v in pairs(list) do
local GroupsOwner = redis:scard(DEVIL.."bot:owners:"..v) or 0
local GroupsMod = redis:scard(DEVIL.."bot:momod:"..v) or 0
local Groupslink = redis:get(DEVIL.."bot:group:link" ..v)
if result.first_name_ then
if #result.first_name_ < 35 then
else
for kkkkk in string.gmatch(result.first_name_, "[^%s]+") do
...
file:write(text)
file:close()
local token_files = 'https://api.telegram.org/bot' .. DEVIL_TOKEN .. '/sendDocument'
...
local botnamess = redis:get(DEVIL.."DEVILnams") or "DEVIL"
local kpSudos1 = redis:scard(DEVIL.."Bot:KpSudos") or "0"
local kpAdmins = redis:scard(DEVIL.."Bot:Admins") or "0"
local kpgban = redis:scard(DEVIL.."bot:gban:") or "0"
local kpvipmems = redis:scard(DEVIL.."bot:vipmems:") or "0"
local kpgps = redis:scard(DEVIL.."bot:groups") or "0"
local kpusers = redis:scard(DEVIL.."bot:userss") or "0"
send(msg.chat_id_, msg.id_, 1, "*- احصائيات البـــــوت ≈*\n*ﮧ -------------------»*\n👨🏼┊اسم البوت ≈ *" .. botnamess .. "*\n🃏┊عدد المطورين ≈ *"..kpSudos1.."*\n🔰┊عدد الادمنيه ≈ *"..kpAdmins.."*\n🔚┊عدد الكروبات ≈ * "..kpgps.."*\n👪┊عدد المشتركين خاص ≈ *" .. kpusers .. "*\n🔔┊عدد المحظورين عام ≈ *"..kpgban.."*\n🔘┊عدد المميزين عام ≈ * "..kpvipmems.."*\n🗯┊قناة السورس ≈ [DEVIL](t.me/DEVIL_ch)\n✓", 1,"md")
end
end
------------------GPS AND USERS-------------------------------------------
if text:match("^المجموعات$") or text:match("^الكروبات$") and is_admin(msg.sender_user_id_, msg.chat_id_) then
local gps = redis:scard(DEVIL.."bot:groups")
local users = redis:scard(DEVIL.."bot:userss")
...
function cb(a,b,c)
local text = b.content_.text_
local users = redis:scard(DEVIL.."bot:userss") or 0
local userss = redis:smembers(DEVIL.."bot:userss") or 0
local msgs = {
[0] = id
}
for i = 1, #userss do
...
if redis:get(DEVIL.."ffosh"..msg.chat_id_) and not is_momod(msg.sender_user_id_, msg.chat_id_) then
local id = msg.id_
local msgs = { [0] = id}
local chat = msg.chat_id_
delete_msg(chat, msgs)
...
send(msg.chat_id_, msg.id_, 1, "🚫┊ الرتبه » (*"..DEVIL3.."*) 🍃\n‏" , 1, "md")
end
...
------------------SOURCE DEVIL---------------------------------------------------
if text:match("^سورس$") or text:match("^السورس$") or text:match("^مطور السورس$") or text:match("^ياسورس$") or  text:match("^سورس DEVIL$") or text:match("^اريد سورس$") then
if not redis:get(DEVIL..'lock:add'..msg.chat_id_) then
local text =  [[
‏
‏
🔍┊ *اهلا بك في سورس DEVIL❗️
...
`git clone https://github.com/alqaser/DEVIL.git && cd DEVIL && chmod +x DEVIL && ./DEVIL`
...
`killall screen && cd DEVIL && screen ./DEVIL`

💬┊ *مطور السورس *≈ [@rr20r](t.me/rr20r)
💲┊ *قناٌة السوٰرس *≈ [@DEVIL_ch](T.ME/DEVIL_ch)
...
redis:set(DEVIL..'welcome:'..msg.chat_id_,welcome)
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmDEVIL(msg).."\n🎟┊ تم وضع الترحيب\n ✓ ", 1, 'md')
end
-----------------welcome------------------------------------------
if text:match("^الترحيب$") and is_momod(msg.sender_user_id_, msg.chat_id_) then
local wel = redis:get(DEVIL..'welcome:'..msg.chat_id_)
if wel then
...
redis:setex(DEVIL.."broadcast" .. msg.chat_id_ .. ":" .. msg.sender_user_id_, 60, true)
...
redis:setex(DEVIL.."broadcast2" .. msg.chat_id_ .. ":" .. msg.sender_user_id_, 60, true)
send(msg.chat_id_, msg.id_, 1, "🌀┊  ارسل لي الرساله الان📲\n🚫┊ للخروج ارسل لي الغاء 📍 ", 1, "md")
end end end
----------------- info bot --------------------------------------------------------------
...
local gps = redis:scard(DEVIL.."bot:groups")
local users = redis:scard(DEVIL.."bot:userss")
local allmgs = redis:get(DEVIL.."bot:allmsgs")
if redis:get(DEVIL.."autoleave") == "On" then
autoleaveAR = "مفعل"
elseif redis:get(DEVIL.."autoleave") == "Off" then
autoleaveAR = "معطل"
elseif not redis:get(DEVIL.."autoleave") then
autoleaveAR = "معطل"
end
if redis:get(DEVIL.."joinbylink") == "On" then
joinbylinkAR = "مفعل"
elseif redis:get(DEVIL.."joinbylink") == "Off" then
joinbylinkAR = "معطل"
elseif not redis:get(DEVIL.."joinbylink") then
joinbylinkAR = "معطل"
end
...
Uptime_1 = redis:get(DEVIL.."bot:Uptime")
...
if not redis:get(DEVIL..'lock:add'..msg.chat_id_) then
local lockDEVIL = {string.match(text, "^(قفل) (.*)$")}
if not is_momod(msg.sender_user_id_, msg.chat_id_)then
send(msg.chat_id_, msg.id_, 1, '💲┊ للأدمنيـــــــه فقــــــــط', 1, 'md')
else
if lockDEVIL[2] == "التعديل" then
if not redis:get(DEVIL.."editmsg" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم قفل التعديل \n🎟┊ الأمر بواسطه » "..tmDEVIL(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ التعديل مقفول سابقا \n🎟┊ الأمر بواسطه » "..tmDEVIL(msg).."\n ‏ ", 1, "md")
end
redis:set(DEVIL..'editmsg'..msg.chat_id_,'delmsg')
end
if lockDEVIL[2] == "الاوامر" then
if not redis:get(DEVIL.."bot:cmds" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم قفل الاوامر \n🎟┊ الأمر بواسطه » "..tmDEVIL(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ الاوامر مقفوله سابقا \n🎟┊ الأمر بواسطه » "..tmDEVIL(msg).."\n ‏ ", 1, "md")
end
redis:set(DEVIL.."bot:cmds" .. msg.chat_id_, true)
end
if lockDEVIL[2] == "البوتات" then
if not redis:get(DEVIL.."bot:bots:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم قفل البوتات \n🎟┊ الأمر بواسطه » "..tmDEVIL(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ البوتات مقفوله سابقا \n🎟┊ الأمر بواسطه » "..tmDEVIL(msg).."\n ‏ ", 1, "md")
end
redis:set(DEVIL.."bot:bots:mute" .. msg.chat_id_, true)
end
if lockDEVIL[2] == "البوتات بالطرد" then
if not redis:get(DEVIL.."bot:botskick" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم قفل البوتات بالطرد \n🎟┊ الأمر بواسطه » "..tmDEVIL(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊البوتات بالطرد مقفوله سابقا \n🎟┊ الأمر بواسطه » "..tmDEVIL(msg).."\n ‏ ", 1, "md")
end
redis:set(DEVIL.."bot:botskick" .. msg.chat_id_, true)
end
if lockDEVIL[2] == "التكرار" then
if not redis:get(DEVIL.."anti-flood:" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم قفل التكرار \n🎟┊ الأمر بواسطه » "..tmDEVIL(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ التكرار مقفول سابقا \n🎟┊ الأمر بواسطه » "..tmDEVIL(msg).."\n ‏ ", 1, "md")
end
redis:set(DEVIL.."anti-flood:" .. msg.chat_id_, true)
end
if lockDEVIL[2] == "التثبيت" then
if not redis:get(DEVIL.."bot:pin:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم قفل التثبيت \n🎟┊ الأمر بواسطه » "..tmDEVIL(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ التثبيت مقفول سابقا \n🎟┊ الأمر بواسطه » "..tmDEVIL(msg).."\n ‏ ", 1, "md")
end
redis:set(DEVIL.."bot:pin:mute" .. msg.chat_id_, true)
end
if lockDEVIL[2] == "الكل" then
if not redis:get(DEVIL.."bot:muteall" .. msg.chat_id_) then
...
redis:set(DEVIL.."bot:bots:mute" .. msg.chat_id_, true)
redis:set(DEVIL.."anti-flood:" .. msg.chat_id_, true)
redis:set(DEVIL.."bot:photo:mute" .. msg.chat_id_, true)
redis:set(DEVIL.."bot:video:mute" .. msg.chat_id_, true)
redis:set(DEVIL.."bot:selfvideo:mute" .. msg.chat_id_, true)
redis:set(DEVIL.."bot:document:mute" .. msg.chat_id_, true)
redis:set(DEVIL.."markdown:lock" .. msg.chat_id_, true)
redis:set(DEVIL.."bot:gifs:mute" .. msg.chat_id_, true)
redis:set(DEVIL.."bot:music:mute" .. msg.chat_id_, true)
redis:set(DEVIL.."bot:voice:mute" .. msg.chat_id_, true)
redis:set(DEVIL.."tags:lock" .. msg.chat_id_, true)
redis:set(DEVIL.."bot:contact:mute" .. msg.chat_id_, true)
redis:set(DEVIL.."bot:tgservice:mute" .. msg.chat_id_, true)
redis:set(DEVIL.."bot:forward:mute" .. msg.chat_id_, true)
redis:set(DEVIL.."ffosh"..msg.chat_id_, true)
end
if lockDEVIL[2] == "الدردشه" then
if not redis:get(DEVIL.."bot:text:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1,  "💬┊ تم قفل الدردشه \n🎟┊ الأمر بواسطه » "..tmDEVIL(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1,  "💬┊ الدردشه مقفوله سابقا \n🎟┊ الأمر بواسطه » "..tmDEVIL(msg).."\n ‏ ", 1, "md")
end
redis:set(DEVIL.."bot:text:mute" .. msg.chat_id_, true)
end
if lockDEVIL[2] == "الصور" then
if not redis:get(DEVIL.."bot:photo:mute" .. msg.chat_id_) then
...
if lockDEVIL[2] == "المشاركه" then
if not redis:get(DEVIL.."bot:duplipost:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1,  "💬┊ تم قفل المشاركه \n🎟┊ الأمر بواسطه » "..tmDEVIL(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1,  "💬┊ المشاركه مقفوله سابقا \n🎟┊ الأمر بواسطه » "..tmDEVIL(msg).."\n ‏ ", 1, "md")
end
redis:set(DEVIL.."bot:duplipost:mute" .. msg.chat_id_, true)
end
if lockDEVIL[2] == "الكلايش" then
if not redis:get(DEVIL.."bot:spam:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1,  "💬┊ تم قفل الكلايش \n🎟┊ الأمر بواسطه » "..tmDEVIL(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1,  "💬┊ الكلايش مقفوله سابقا \n🎟┊ الأمر بواسطه » "..tmDEVIL(msg).."\n ‏ ", 1, "md")
end
redis:set(DEVIL.."bot:spam:mute" .. msg.chat_id_, true)
end
if lockDEVIL[2] == "الفيديو" then
if not redis:get(DEVIL.."bot:video:mute" .. msg.chat_id_) then
...
if lockDEVIL[2] == "السيلفي" then
if not redis:get(DEVIL.."bot:selfvideo:mute" .. msg.chat_id_) then
...
if lockDEVIL[2] == "الفايلات" then
if not redis:get(DEVIL.."bot:document:mute" .. msg.chat_id_) then
...
if lockDEVIL[2] == "الالعاب" then
if not redis:get(DEVIL.."Game:lock" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم قفل الالعاب \n🎟┊ الأمر بواسطه » "..tmDEVIL(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ الالعاب مقفوله سابقا \n🎟┊ الأمر بواسطه » "..tmDEVIL(msg).."\n ‏ ", 1, "md")
end
redis:set(DEVIL.."Game:lock" .. msg.chat_id_, true)
end
if lockDEVIL[2] == "الانلاين" then
if not redis:get(DEVIL.."bot:inline:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم قفل الانلاين \n🎟┊ الأمر بواسطه » "..tmDEVIL(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ الانلاين مقفوله سابقا \n🎟┊ الأمر بواسطه » "..tmDEVIL(msg).."\n ‏ ", 1, "md")
end
redis:set(DEVIL.."bot:inline:mute" .. msg.chat_id_, true)
end
if lockDEVIL[2] == "البوست" then
if not redis:get(DEVIL.."post:lock" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم قفل البوست \n🎟┊ الأمر بواسطه » "..tmDEVIL(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ البوست مقفوله سابقا \n🎟┊ الأمر بواسطه » "..tmDEVIL(msg).."\n ‏ ", 1, "md")
end
redis:set(DEVIL.."post:lock" .. msg.chat_id_, true)
end
if lockDEVIL[2] == "الماركدون" then
if not redis:get(DEVIL.."markdown:lock" .. msg.chat_id_) then
...
if lockDEVIL[2] == "المتحركه" then
if not redis:get(DEVIL.."bot:gifs:mute" .. msg.chat_id_) then
...
if lockDEVIL[2] == "الاغاني" then
if not redis:get(DEVIL.."bot:music:mute" .. msg.chat_id_) then
...
if lockDEVIL[2] == "الصوت" then
if not redis:get(DEVIL.."bot:voice:mute" .. msg.chat_id_) then
...
if lockDEVIL[2] == "الروابط" then
if not redis:get(DEVIL.."bot:links:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم قفل الروابط \n🎟┊ الأمر بواسطه » "..tmDEVIL(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ الروابط مقفوله سابقا \n🎟┊ الأمر بواسطه » "..tmDEVIL(msg).."\n ‏ ", 1, "md")
end
redis:set(DEVIL.."bot:links:mute" .. msg.chat_id_, true)
end
if lockDEVIL[2] == "المواقع" then
if not redis:get(DEVIL.."bot:location:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم قفل المواقع \n🎟┊ الأمر بواسطه » "..tmDEVIL(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ المواقع مقفوله سابقا \n🎟┊ الأمر بواسطه » "..tmDEVIL(msg).."\n ‏ ", 1, "md")
end
redis:set(DEVIL.."bot:location:mute" .. msg.chat_id_, true)
end
if lockDEVIL[2] == "التاك" then
if not redis:get(DEVIL.."tags:lock" .. msg.chat_id_) then
...
if lockDEVIL[2] == "الهاشتاك" then
if not redis:get(DEVIL.."bot:hashtag:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم قفل الهاشتاك \n🎟┊ الأمر بواسطه » "..tmDEVIL(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ الهاشتاك مقفوله سابقا \n🎟┊ الأمر بواسطه » "..tmDEVIL(msg).."\n ‏ ", 1, "md")
end
redis:set(DEVIL.."bot:hashtag:mute" .. msg.chat_id_, true)
end
if lockDEVIL[2] == "الجهات" then
if not redis:get(DEVIL.."bot:contact:mute" .. msg.chat_id_) then
...
if lockDEVIL[2] == "الويب" then
if not redis:get(DEVIL.."bot:webpage:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم قفل الويب \n🎟┊ الأمر بواسطه » "..tmDEVIL(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ الويب مقفوله سابقا \n🎟┊ الأمر بواسطه » "..tmDEVIL(msg).."\n ‏ ", 1, "md")
end
redis:set(DEVIL.."bot:webpage:mute" .. msg.chat_id_, true)
end
if lockDEVIL[2] == "العربيه" then
if not redis:get(DEVIL.."bot:arabic:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم قفل العربيه \n🎟┊ الأمر بواسطه » "..tmDEVIL(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ العربيه مقفوله سابقا \n🎟┊ الأمر بواسطه » "..tmDEVIL(msg).."\n ‏ ", 1, "md")
end
redis:set(DEVIL.."bot:arabic:mute" .. msg.chat_id_, true)
end
if lockDEVIL[2] == "الانظمام" then
if not redis:get(DEVIL.."bot:member:lock" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم قفل الانظمام \n🎟┊ الأمر بواسطه » "..tmDEVIL(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ الانظمام مقفوله سابقا \n🎟┊ الأمر بواسطه » "..tmDEVIL(msg).."\n ‏ ", 1, "md")
end
redis:set(DEVIL.."bot:member:lock" .. msg.chat_id_, true)
end
if lockDEVIL[2] == "الانكليزيه" then
if not redis:get(DEVIL.."bot:english:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم قفل الانكليزي \n🎟┊ الأمر بواسطه » "..tmDEVIL(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ الانكليزيه مقفوله سابقا \n🎟┊ الأمر بواسطه » "..tmDEVIL(msg).."\n ‏ ", 1, "md")
end
redis:set(DEVIL.."bot:english:mute" .. msg.chat_id_, true)
end
if lockDEVIL[2] == "الاشعارات" then
if not redis:get(DEVIL.."bot:tgservice:mute" .. msg.chat_id_) then
...
if lockDEVIL[2] == "الملصقات" then
if not redis:get(DEVIL.."bot:sticker:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم قفل الملصقات \n🎟┊ الأمر بواسطه » "..tmDEVIL(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ الملصقات مقفوله سابقا \n🎟┊ الأمر بواسطه » "..tmDEVIL(msg).."\n ‏ ", 1, "md")
end
redis:set(DEVIL.."bot:sticker:mute" .. msg.chat_id_, true)
end
if lockDEVIL[2] == "التوجيه" then
if not redis:get(DEVIL.."bot:forward:mute" .. msg.chat_id_) then
...
if lockDEVIL[2] == "الفشار" then
if not redis:get(DEVIL.."ffosh" .. msg.chat_id_) then
...
if not redis:get(DEVIL..'lock:add'..msg.chat_id_) then
local UNDEVIL = { string.match(text, "^(فتح) (.*)$") }
if not is_momod(msg.sender_user_id_, msg.chat_id_)then
send(msg.chat_id_, msg.id_, 1, '💲┊ للأدمنيـــــــه فقــــــــط', 1, 'md')
else
if UNDEVIL[2] == "التعديل" then
if redis:get(DEVIL.."editmsg" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم فتح التعديل \n🎟┊ الأمر بواسطه » "..tmDEVIL(msg).."\n ‏ ", 1, "md")
redis:del(DEVIL.."editmsg" .. msg.chat_id_)
else
send(msg.chat_id_, msg.id_, 1, "💬┊ التعديل مفتوح سابقا \n🎟┊ الأمر بواسطه » "..tmDEVIL(msg).."\n ‏ ", 1, "md")
end
end
if UNDEVIL[2] == "الاوامر" then
if redis:get(DEVIL.."bot:cmds" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم فتح الاوامر \n🎟┊ الأمر بواسطه » "..tmDEVIL(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ الاوامر مفتوحه سابقا \n🎟┊ الأمر بواسطه » "..tmDEVIL(msg).."\n ‏ ", 1, "md")
end
redis:del(DEVIL.."bot:cmds" .. msg.chat_id_)
end
if UNDEVIL[2] == "البوتات" then
if redis:get(DEVIL.."bot:bots:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم فتح البوتات \n🎟┊ الأمر بواسطه » "..tmDEVIL(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ البوتات مفتوحه سابقا \n🎟┊ الأمر بواسطه » "..tmDEVIL(msg).."\n ‏ ", 1, "md")
end
redis:del(DEVIL.."bot:bots:mute" .. msg.chat_id_)
end
if UNDEVIL[2] == "البوتات بالطرد" then
if redis:get(DEVIL.."bot:botskick" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم فتح البوتات بالطرد \n🎟┊ الأمر بواسطه » "..tmDEVIL(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ البوتات بالطرد مفتوحه سابقا \n🎟┊ الأمر بواسطه » "..tmDEVIL(msg).."\n ‏ ", 1, "md")
end
redis:del(DEVIL.."bot:botskick" .. msg.chat_id_)
end
if UNDEVIL[2] == "التكرار" then
if redis:get(DEVIL.."anti-flood:" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم فتح التكرار \n🎟┊ الأمر بواسطه » "..tmDEVIL(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ التكرار مفتوح سابقا \n🎟┊ الأمر بواسطه » "..tmDEVIL(msg).."\n ‏ ", 1, "md")
end
redis:del(DEVIL.."anti-flood:" .. msg.chat_id_)
end
if UNDEVIL[2] == "التثبيت" then
if redis:get(DEVIL.."bot:pin:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم فتح التثبيت \n🎟┊ الأمر بواسطه » "..tmDEVIL(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ التثبيت مفتوح سابقا \n🎟┊ الأمر بواسطه » "..tmDEVIL(msg).."\n ‏ ", 1, "md")
end
redis:del(DEVIL.."bot:pin:mute" .. msg.chat_id_)
end
if UNDEVIL[2] == "الكل" then
if redis:get(DEVIL.."bot:muteall" .. msg.chat_id_) then
...
redis:del(DEVIL.."bot:bots:mute" .. msg.chat_id_)
redis:del(DEVIL.."anti-flood:" .. msg.chat_id_)
redis:del(DEVIL.."bot:photo:mute" .. msg.chat_id_)
redis:del(DEVIL.."bot:video:mute" .. msg.chat_id_)
redis:del(DEVIL.."bot:selfvideo:mute" .. msg.chat_id_)
redis:del(DEVIL.."bot:document:mute" .. msg.chat_id_)
redis:del(DEVIL.."markdown:lock" .. msg.chat_id_)
redis:del(DEVIL.."bot:gifs:mute" .. msg.chat_id_)
redis:del(DEVIL.."bot:music:mute" .. msg.chat_id_)
redis:del(DEVIL.."bot:voice:mute" .. msg.chat_id_)
redis:del(DEVIL.."tags:lock" .. msg.chat_id_)
redis:del(DEVIL.."bot:contact:mute" .. msg.chat_id_)
redis:del(DEVIL.."bot:tgservice:mute" .. msg.chat_id_)
redis:del(DEVIL.."bot:forward:mute" .. msg.chat_id_)
redis:del(DEVIL.."ffosh"..msg.chat_id_)
end
if UNDEVIL[2] == "الدردشه" then
if redis:get(DEVIL.."bot:text:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1,  "💬┊ تم فتح الدردشه \n🎟┊ الأمر بواسطه » "..tmDEVIL(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1,  "💬┊ الدردشه مفتوحه سابقا \n🎟┊ الأمر بواسطه » "..tmDEVIL(msg).."\n ‏ ", 1, "md")
end
redis:del(DEVIL.."bot:text:mute" .. msg.chat_id_)
end
if UNDEVIL[2] == "الصور" then
if redis:get(DEVIL.."bot:photo:mute" .. msg.chat_id_) then
...
if UNDEVIL[2] == "المشاركه" then
if redis:get(DEVIL.."bot:duplipost:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1,  "💬┊ تم فتح المشاركه \n🎟┊ الأمر بواسطه » "..tmDEVIL(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1,  "💬┊ المشاركه مفتوحه سابقا \n🎟┊ الأمر بواسطه » "..tmDEVIL(msg).."\n ‏ ", 1, "md")
end
redis:del(DEVIL.."bot:duplipost:mute" .. msg.chat_id_)
end
if UNDEVIL[2] == "الكلايش" then
if redis:get(DEVIL.."bot:spam:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1,  "💬┊ تم فتح الكلايش \n🎟┊ الأمر بواسطه » "..tmDEVIL(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1,  "💬┊ الكلايش مفتوحه سابقا \n🎟┊ الأمر بواسطه » "..tmDEVIL(msg).."\n ‏ ", 1, "md")
end
redis:del(DEVIL.."bot:spam:mute" .. msg.chat_id_)
end
if UNDEVIL[2] == "الفيديو" then
if redis:get(DEVIL.."bot:video:mute" .. msg.chat_id_) then
...
if UNDEVIL[2] == "الالعاب" then
if redis:get(DEVIL.."Game:lock" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم فتح الالعاب \n🎟┊ الأمر بواسطه » "..tmDEVIL(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ الالعاب مفتوحه سابقا \n🎟┊ الأمر بواسطه » "..tmDEVIL(msg).."\n ‏ ", 1, "md")
end
redis:del(DEVIL.."Game:lock" .. msg.chat_id_)
end
if UNDEVIL[2] == "الانلاين" then
if redis:get(DEVIL.."bot:inline:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم فتح الانلاين \n🎟┊ الأمر بواسطه » "..tmDEVIL(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ الانلاين مفتوحه سابقا \n🎟┊ الأمر بواسطه » "..tmDEVIL(msg).."\n ‏ ", 1, "md")
end
redis:del(DEVIL.."bot:inline:mute" .. msg.chat_id_)
end
if UNDEVIL[2] == "البوست" then
if redis:get(DEVIL.."post:lock" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم فتح البوست \n🎟┊ الأمر بواسطه » "..tmDEVIL(msg).."\n ‏ ", 1, "md")            
else
send(msg.chat_id_, msg.id_, 1, "💬┊ البوست مفتوح سابقا \n🎟┊ الأمر بواسطه » "..tmDEVIL(msg).."\n ‏ ", 1, "md")
end
redis:del(DEVIL.."post:lock" .. msg.chat_id_)
end
if UNDEVIL[2] == "الماركدون" then
if redis:get(DEVIL.."markdown:lock" .. msg.chat_id_) then
...
if UNDEVIL[2] == "الروابط" then
if redis:get(DEVIL.."bot:links:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم فتح الروابط \n🎟┊ الأمر بواسطه » "..tmDEVIL(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ الروابط مفتوحه سابقا \n🎟┊ الأمر بواسطه » "..tmDEVIL(msg).."\n ‏ ", 1, "md")
end
redis:del(DEVIL.."bot:links:mute" .. msg.chat_id_)
end
if UNDEVIL[2] == "المواقع" then
if redis:get(DEVIL.."bot:location:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم فتح المواقع \n🎟┊ الأمر بواسطه » "..tmDEVIL(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ المواقع مفتوحه سابقا \n🎟┊ الأمر بواسطه » "..tmDEVIL(msg).."\n ‏ ", 1, "md")
end
redis:del(DEVIL.."bot:location:mute" .. msg.chat_id_)
end
if UNDEVIL[2] == "التاك" then
if redis:get(DEVIL.."tags:lock" .. msg.chat_id_) then
...
if UNDEVIL[2] == "الهاشتاك" then
if redis:get(DEVIL.."bot:hashtag:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم فتح الهاشتاك \n🎟┊ الأمر بواسطه » "..tmDEVIL(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ الهاشتاك مفتوحه سابقا \n🎟┊ الأمر بواسطه » "..tmDEVIL(msg).."\n ‏ ", 1, "md")
end
redis:del(DEVIL.."bot:hashtag:mute" .. msg.chat_id_)
end
if UNDEVIL[2] == "الجهات" then
if redis:get(DEVIL.."bot:contact:mute" .. msg.chat_id_) then
...
if UNDEVIL[2] == "الويب" then
if redis:get(DEVIL.."bot:webpage:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم فتح الويب \n🎟┊ الأمر بواسطه » "..tmDEVIL(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ الويب مفتوح سابقا \n🎟┊ الأمر بواسطه » "..tmDEVIL(msg).."\n ‏ ", 1, "md")
end
redis:del(DEVIL.."bot:webpage:mute" .. msg.chat_id_)
end
if UNDEVIL[2] == "العربيه" then
if redis:get(DEVIL.."bot:arabic:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم فتح العربيه \n🎟┊ الأمر بواسطه » "..tmDEVIL(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ العربيه مفتوحه سابقا \n🎟┊ الأمر بواسطه » "..tmDEVIL(msg).."\n ‏ ", 1, "md")
end
redis:del(DEVIL.."bot:arabic:mute" .. msg.chat_id_)
end
if UNDEVIL[2] == "الانظمام" then
if redis:get(DEVIL.."bot:member:lock" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم فتح الانظمام \n🎟┊ الأمر بواسطه » "..tmDEVIL(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ الانظمام مفتوح سابقا\n🎟┊ الأمر بواسطه » "..tmDEVIL(msg).."\n ‏ ", 1, "md")
end
redis:del(DEVIL.."bot:member:lock" .. msg.chat_id_)
end
if UNDEVIL[2] == "الانكليزيه" then
if redis:get(DEVIL.."bot:english:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم فتح الانكليزي \n🎟┊ الأمر بواسطه » "..tmDEVIL(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ الانكليزيه مفتوحه سابقا \n🎟┊ الأمر بواسطه » "..tmDEVIL(msg).."\n ‏ ", 1, "md")
end
redis:del(DEVIL.."bot:english:mute" .. msg.chat_id_)
end
if UNDEVIL[2] == "الاشعارات" then
if redis:get(DEVIL.."bot:tgservice:mute" .. msg.chat_id_) then
...
if UNDEVIL[2] == "الملصقات" then
if redis:get(DEVIL.."bot:sticker:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم فتح الملصقات \n🎟┊ الأمر بواسطه » "..tmDEVIL(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ الملصقات مفتوحه سابقا\n🎟┊ الأمر بواسطه » "..tmDEVIL(msg).."\n ‏ ", 1, "md")
end
redis:del(DEVIL.."bot:sticker:mute" .. msg.chat_id_)
end
if UNDEVIL[2] == "التوجيه" then
if redis:get(DEVIL.."bot:forward:mute" .. msg.chat_id_) then
...
if UNDEVIL[2] == "الفشار" then
if redis:get(DEVIL.."ffosh"..msg.chat_id_) then
...
redis:del(DEVIL.."keed_text" .. msg.chat_id_)
end
...
redis:del(DEVIL.."keed_photo" .. msg.chat_id_)
end
...
redis:del(DEVIL.."keed_video" .. msg.chat_id_)
end
...
redis:del(DEVIL.."keed_Document" .. msg.chat_id_)
end
...
redis:del(DEVIL.."keed_markdon" .. msg.chat_id_)
end
...
redis:del(DEVIL.."keed_gif" .. msg.chat_id_)
end
...
redis:del(DEVIL.."keed_audio" .. msg.chat_id_)
end
...
redis:del(DEVIL.."keed_voice" .. msg.chat_id_)
end
...
redis:del(DEVIL.."keed_link" .. msg.chat_id_)
end
...
redis:del(DEVIL.."keed_user" .. msg.chat_id_)
end
...
redis:del(DEVIL.."keed_hashtag" .. msg.chat_id_)
end
...
redis:del(DEVIL.."keed_contect" .. msg.chat_id_)
end
...
redis:del(DEVIL.."keed_arbic" .. msg.chat_id_)
end
...
redis:del(DEVIL.."keed_fosh" .. msg.chat_id_)
end
...
redis:set(DEVIL.."bot:sens:spam" .. msg.chat_id_, sensspam[2])
...
local kpbanned = redis:scard(DEVIL.."bot:banned:" .. msg.chat_id_) or "0"
redis:del(DEVIL.."bot:banned:" .. msg.chat_id_)
...
redis:setex(DEVIL.."CleanBlockList" .. msg.chat_id_ .. ":" .. msg.sender_user_id_, 35, true)
end end
----------------------------------------------
...
local kpmomod = redis:scard(DEVIL.."bot:momod:" .. msg.chat_id_) or "0"
send(msg.chat_id_, msg.id_, 1, "📉┊عدد الأدمنيه ≈ *"..kpmomod.."*\n🌀┊ تـم مسحهم بنجـــــــــــاح ✓\n‏", 1, "md")
redis:del(DEVIL.."bot:momod:" .. msg.chat_id_)
end end
-----------------------------------------------
...
local kpowners = redis:scard(DEVIL.."bot:owners:" .. msg.chat_id_) or "0"
send(msg.chat_id_, msg.id_, 1, "📉┊عدد المدراء ≈ *"..kpowners.."*\n🌀┊ تـم مسحهم بنجـــــــــــاح ✓\n‏", 1, "md")
redis:del(DEVIL.."bot:owners:" .. msg.chat_id_)
end end
-----------------------------------------------
...
local kpmonshis = redis:scard(DEVIL.."bot:monshis:" .. msg.chat_id_) or "0"
send(msg.chat_id_, msg.id_, 1, "📉┊عدد المنشئين ≈ *"..kpmonshis.."*\n🌀┊ تـم مسحهم بنجـــــــــــاح ✓\n‏", 1, "md")
redis:del(DEVIL.."bot:monshis:" .. msg.chat_id_)
end end
--------DEL DEVIL_SUDO-----------------------------------------
...
local kpvipmem = redis:scard(DEVIL.."bot:vipmem:" .. msg.chat_id_) or "0"
send(msg.chat_id_, msg.id_, 1, "📉┊عدد المميزين ≈ *"..kpvipmem.."*\n🌀┊ تـم مسحهم بنجـــــــــــاح ✓\n‏", 1, "md")
redis:del(DEVIL.."bot:vipmem:" .. msg.chat_id_)
end  end
---------------keeed del --------------------------------------------
...
local kpkeed = redis:scard(DEVIL.."bot:keed:" .. msg.chat_id_) or "0"
send(msg.chat_id_, msg.id_, 1, "📉┊عدد المقيدين ≈ *"..kpkeed.."*\n🌀┊ تـم مسحهم بنجـــــــــــاح ✓\n‏", 1, "md")
local hash = "bot:keed:" .. msg.chat_id_
local list = redis:smembers(DEVIL..hash)
for k, v in pairs(list) do
redis:del(DEVIL.."bot:keed:" .. msg.chat_id_)
HTTPS.request("https://api.telegram.org/bot"..DEVIL_TOKEN.."/restrictChatMember?chat_id=" ..msg.chat_id_.. "&user_id=" ..v.."&can_send_messages=True&can_send_media_messages=True&can_send_other_messages=True&can_add_web_page_previews=True")
end end end
------------DEL vipmems-------------------------------------------
...
local kpmuted = redis:scard(DEVIL.."bot:muted:" .. msg.chat_id_) or "0"
redis:del(DEVIL.."bot:muted:" .. msg.chat_id_)
...
if redis:get(DEVIL.."CleanBlockList" .. msg.chat_id_ .. ":" .. msg.sender_user_id_) then
local d = 0
if text:match("^1$") then
redis:del(DEVIL.."CleanBlockList" .. msg.chat_id_ .. ":" .. msg.sender_user_id_)
for i = 1, NumberReturn do
getChannelMembers(msg.chat_id_, d, "Kicked", 200, kickedlist)
d = d + 200
end
if redis:get(DEVIL.."lang:gp:" .. msg.chat_id_) then
...
if not redis:get(DEVIL.."flood:max:" .. msg.chat_id_) then
flood_m = 5
else
flood_m = redis:get(DEVIL.."flood:max:" .. msg.chat_id_)
end
if not redis:get(DEVIL.."bot:sens:spam" .. msg.chat_id_) then
spam_c = 400
else
spam_c = redis:get(DEVIL.."bot:sens:spam" .. msg.chat_id_)
end
if redis:get(DEVIL.."warn:max:" .. msg.chat_id_) then
sencwarn = tonumber(redis:get(DEVIL.."warn:max:" .. msg.chat_id_))
else
sencwarn = 4
end
if redis:get(DEVIL.."floodstatus" .. msg.chat_id_) == "DelMsg" then
floodstatus = "المسح"
elseif redis:get(DEVIL.."floodstatus" .. msg.chat_id_) == "Kicked" then
floodstatus = "الطرد"
elseif not redis:get(DEVIL.."floodstatus" .. msg.chat_id_) then
floodstatus = "الطرد"
end
if redis:get(DEVIL.."warnstatus" .. msg.chat_id_) == "Muteuser" then
warnstatus = "الكتم"
elseif redis:get(DEVIL.."warnstatus" .. msg.chat_id_) == "Remove" then
warnstatus = "الطرد"
elseif not redis:get(DEVIL.."warnstatus" .. msg.chat_id_) then
warnstatus = "الكتم"
end
if redis:get(DEVIL.."bot:music:mute" .. msg.chat_id_) then
mute_music = "✅"
...
if redis:get(DEVIL.."bot:welcome" .. msg.chat_id_) then
send_welcome = "✅"
else
send_welcome = "✖️"
end
...
os.execute('wget https://raw.githubusercontent.com/alqaser/DEVIL/master/DEVIL.lua') 
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmDEVIL(msg).."\n🎟┊ تم تحديث السورس \n ✓ ", 1, 'md')
dofile('DEVIL.lua')  
...
if not redis:get(DEVIL..'lock:add'..msg.chat_id_) then
redis:setex(DEVIL.."rules" .. msg.chat_id_ .. ":" .. msg.sender_user_id_, 60, true)
...
if not redis:get(DEVIL..'lock:add'..msg.chat_id_) then
redis:set(DEVIL..'bot:setphoto'..msg.chat_id_..':'..msg.sender_user_id_,true)
...
if redis:get(DEVIL.."lang:gp:" .. result.id_) then
send(result.id_, 0, 1, "🌀┊  تم اخراج البوت 🎐\n🔱┊ راسل المطور للتفعيل 📮\n🔰┊ *المطور* : [" .. UserDEVIL .. "]\n‏", 1, "md")
else
send(result.id_, 0, 1, "🌀┊  تم اخراج البوت 🎐\n🔱┊ راسل المطور للتفعيل 📮\n🔰┊ *المطور* : [" .. UserDEVIL .. "]\n‏", 1, "md")
end
chat_leave(result.id_, bot_id)
redis:srem(DEVIL.."bot:groups", result.id_)
else
...
local gp = tonumber(redis:scard(DEVIL.."bot:userss"))
gps = redis:smembers(DEVIL.."bot:userss")
...
if not redis:get(DEVIL.."dellmssg"..msg.chat_id_) and not is_owner(msg.sender_user_id_, msg.chat_id_) then
...
if not redis:get(DEVIL..'lock:add'..msg.chat_id_) then
if not redis:get(DEVIL.."lock_pinn"..msg.chat_id_) and not is_owner(msg.sender_user_id_, msg.chat_id_) then
...
redis:set(DEVIL.."pinnedmsg" .. msg.chat_id_, msg.reply_to_message_id_)
...
function gp_DEVIL_info(arg,data)
local list = redis:smembers(DEVIL.."bot:owners:" .. msg.chat_id_)
if list[1] or list[2] or list[3] or list[4] then
user_info = redis:get(DEVIL.."user:Name" .. (list[1] or list[2] or list[3] or list[4]))
end
...
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmDEVIL(msg).."\n🎟┊ تم مسح الرد بنجاح\n ✓ ", 1, 'md')
redis:del(DEVIL..'add:repgp'..msg.sender_user_id_..''..msg.chat_id_..'')
redis:del(DEVIL..'gif_repgp'..msg.content_.text_..''..msg.chat_id_..'')
redis:del(DEVIL..'voice_repgp'..msg.content_.text_..''..msg.chat_id_..'')
redis:del(DEVIL..'stecker_repgp'..msg.content_.text_..''..msg.chat_id_..'')
redis:del(DEVIL..'video_repgp'..msg.content_.text_..''..msg.chat_id_..'')
redis:del(DEVIL..'text_repgp'..msg.content_.text_..''..msg.chat_id_..'')
redis:del(DEVIL..'rep_owner'..msg.content_.text_..''..msg.chat_id_..'')
return false
end
end
--------------------------------------------------------------------------
if msg.content_.text_ == 'اضف رد' and is_owner(msg.sender_user_id_, msg.chat_id_)  then
redis:set(DEVIL..'add:repgp'..msg.sender_user_id_..''..msg.chat_id_..'','set_repgp')
send(msg.chat_id_, msg.id_, 1, '📌┊ ارسل لي كلمه الرد الان ❗️\n',1, 'md')
return false    end
if msg.content_.text_ then
local content_keep = redis:get(DEVIL..'add:repgp'..msg.sender_user_id_..''..msg.chat_id_..'')
if content_keep == 'set_repgp' then
send(msg.chat_id_, msg.id_, 1, '📌┊ ارسل جواب الرد قد يكون\n🗯┊ {نص-ملصق-بصمه...} ❗️\n' ,  1, 'md')
redis:set(DEVIL..'add:repgp'..msg.sender_user_id_..''..msg.chat_id_..'','save_repgp')
redis:set(DEVIL..'addreplaygp:'..msg.sender_user_id_..''..msg.chat_id_..'',msg.content_.text_)
redis:sadd(DEVIL..'rep_owner'..msg.chat_id_..'',msg.content_.text_)
return false
end
end
-------------------------------------------------------------------------
if  msg.content_.text_ == 'الردود' and is_owner(msg.sender_user_id_, msg.chat_id_) then
local redod = redis:smembers(DEVIL..'rep_owner'..msg.chat_id_..'')
if #redod == 0 then
...
for k,v in pairs(redod) do
redis:del(DEVIL..'add:repgp'..msg.sender_user_id_..''..msg.chat_id_..'')
redis:del(DEVIL..'gif_repgp'..v..msg.chat_id_)
redis:del(DEVIL..'voice_repgp'..v..msg.chat_id_)
redis:del(DEVIL..'stecker_repgp'..v..msg.chat_id_)
redis:del(DEVIL..'video_repgp'..v..msg.chat_id_)
redis:del(DEVIL..'text_repgp'..v..msg.chat_id_)
redis:del(DEVIL..'rep_owner'..msg.chat_id_..'',msg.content_.text_)
end
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmDEVIL(msg).."\n🎟┊ تم مسح جميع الردود\n ✓ ", 1, 'md')
return false
end
...
redis:set(DEVIL.."add:repallt"..msg.sender_user_id_,'del_rep1')
send(msg.chat_id_, msg.id_, 1, "📌┊ ارسل لي كلمه الرد لمسحها ❗️\n",1, "md")
return false
end
if msg.content_.text_ then
...
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmDEVIL(msg).."\n🎟┊ تم مسح الرد للكل\n ✓ ", 1, 'md')
redis:del(DEVIL.."add:repallt"..msg.sender_user_id_)
redis:del(DEVIL.."gif_repall"..msg.content_.text_)
redis:del(DEVIL.."voice_repall"..msg.content_.text_)
redis:del(DEVIL.."stecker_repall"..msg.content_.text_)
redis:del(DEVIL.."video_repall"..msg.content_.text_)
redis:del(DEVIL.."text_repall"..msg.content_.text_)
redis:del(DEVIL.."rep_sudo",msg.content_.text_)
return false
end
end
--------------------------------------------------------------------------
if msg.content_.text_ == 'اضف رد للكل' and is_sudo(msg)  then
redis:set(DEVIL.."add:repallt"..msg.sender_user_id_,'set_rep')
send(msg.chat_id_, msg.id_, 1, "📌┊ ارسل لي كلمه الرد ❗️\n",1, "md")
return false    end
if msg.content_.text_ then
local content_keep = redis:get(DEVIL.."add:repallt"..msg.sender_user_id_)
if content_keep == 'set_rep' then
send(msg.chat_id_, msg.id_, 1, "📌┊ ارسل جواب الرد قد يكون\n🗯┊ {نص-ملصق-بصمه...} ❗️\n" ,  1, "md")
redis:set(DEVIL.."add:repallt"..msg.sender_user_id_,'save_rep')
redis:set(DEVIL.."addreply2:"..msg.sender_user_id_, msg.content_.text_)
redis:sadd(DEVIL.."rep_sudo",msg.content_.text_)
return false
end    end
------------------------------------------------------------------------------------
if  msg.content_.text_ == "ردود المطور" and is_sudo(msg) then
local redod = redis:smembers(DEVIL.."rep_sudo")
if #redod == 0 then
...
for k,v in pairs(redod) do
redis:del(DEVIL.."add:repallt"..v)
redis:del(DEVIL.."gif_repall"..v)
redis:del(DEVIL.."voice_repall"..v)
redis:del(DEVIL.."stecker_repall"..v)
redis:del(DEVIL.."video_repall"..v)
redis:del(DEVIL.."text_repall"..v)
redis:del(DEVIL.."rep_sudo",msg.content_.text_)
end
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmDEVIL(msg).."\n🎟┊ تم مسح ردود المطور\n ✓ ", 1, 'md')
return false
...
local list = redis:smembers(DEVIL.."bot:owners:" .. chattid)
if list[1] or list[2] or list[3] or list[4] then
user_info = redis:get(DEVIL.."user:Name" .. (list[1] or list[2] or list[3] or list[4]))
end
if user_info then
...
local kpmonshis = redis:scard(DEVIL.."bot:monshis:" .. chattid) or "0"
local kpbanned = redis:scard(DEVIL.."bot:banned:" .. chattid) or "0"
local kpowners = redis:scard(DEVIL.."bot:owners:" .. chattid) or "0"
local kpmuted = redis:scard(DEVIL.."bot:muted:" .. chattid) or "0"
local kpkeed = redis:scard(DEVIL.."bot:keed:" .. chattid) or "0"
local kpmomod = redis:scard(DEVIL.."bot:momod:" .. chattid) or "0"
local kpvipmem = redis:scard(DEVIL.."bot:vipmem:" .. chattid) or "0"
if not redis:get(DEVIL.."bot:group:link"..chattid) then
local getlink = 'https://api.telegram.org/bot'..DEVIL_TOKEN..'/exportChatInviteLink?chat_id='..chattid
local req = https.request(getlink)
local link = KPJS:decode(req)
if link.ok == true then 
redis:set(DEVIL.."bot:group:link"..chattid,link.result)
end
end
local lik_1 = redis:get(DEVIL.."bot:group:link"..chattid)
if lik_1 then
link = lik_1 
else
link = link.result
end
send(msg.chat_id_, msg.id_, 1, "🕴┊ المدير » ["..owner.."]\n🥈┊ﮧ ["..title_name(chattid).."]("..(link or "t.me/DEVIL_ch")..")\n©️┊ عدد المــــدراء   » *"..kpowners.."*\n©️┊ عدد المنشئين   » *"..kpmonshis.."*\n🔆┊ عدد الادمنيـــه   » *"..kpmomod.."*\n🔰┊ عدد المكتومين  » *"..kpmuted.."*\n®️┊ عدد المحظورين » *"..kpbanned.."*\n®️┊ عدد المقيديــن   » *"..kpkeed.."*\n🗯┊ عدد المميزيـــن  » *"..kpvipmem.."*\n💠┊ﮧ `"..chattid.."`\n🏮┊ المطور » ["..sudo.."]\n✓", 1,"md")
end
tdcli_function ({
ID = "GetChannelFull",
...
getChannelFull(chattid, gp_DEVIL_info, nil)
end 
end
------------UNPIN-----------------------------------------------
...
if not redis:get(DEVIL..'lock:add'..msg.chat_id_) then
tdcli.sendDocument(DEVIL_SUDO, 0, 0, 1, nil, './DEVIL.lua', dl_cb, nil)
...
pinmsg(msg.chat_id_, pin_id, 0)
elseif redis:get(DEVIL.."lang:gp:" .. msg.chat_id_) then
...
if not redis:get(DEVIL.."lock_kickme"..msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "🌀┊ امر اطردني معطل 🍃", 1, "md")
return false
end
redis:set(DEVIL.."kick_me"..msg.sender_user_id_..""..msg.chat_id_.."","kick_mee")
redis:set(DEVIL.."unkick_me"..msg.sender_user_id_..""..msg.chat_id_.."","kick_no")
send(msg.chat_id_, msg.id_, 1, "🔰┊ ارسل *{ نعم }* لطردك\n🚫┊ ارسل *{ لا } *لالغاء طردك", 1, "md")
end
local Kpkick = redis:get(DEVIL.."kick_me"..msg.sender_user_id_..""..msg.chat_id_.."")
...
if Kpkick == "kick_mee" then
chat_kick(msg.chat_id_, msg.sender_user_id_)
redis:del(DEVIL.."kick_me"..msg.sender_user_id_..""..msg.chat_id_.."","kick_mee")
redis:del(DEVIL.."unkick_me"..msg.sender_user_id_..""..msg.chat_id_.."","kick_no")
send(msg.chat_id_, msg.id_, 1, "🔰┊ تم طردتك حياتي", 1, "md")
end 
end
end
if text:match("^لا$") then
local Kpunkick = redis:get(DEVIL.."unkick_me"..msg.sender_user_id_..""..msg.chat_id_.."")
...
if not redis:get(DEVIL..'lock:add'..msg.chat_id_) then
redis:set(DEVIL.."bot:viewget" .. msg.sender_user_id_, true)
...
redis:set(DEVIL.."addstart1"..msg.sender_user_id_, "theaddstarts")
return false
end end
if text then
local theaddstart = redis:get(DEVIL.."addstart1"..msg.sender_user_id_)
if theaddstart == 'theaddstarts' then
send(msg.chat_id_, msg.id_, 1, "🌀┊ تم حفظ الكليشه 🍃", 1, 'md')
redis:del(DEVIL.."addstart1"..msg.sender_user_id_)
redis:set(DEVIL.."startbot", text)
return false
end end
...
if not redis:get(DEVIL..'lock:add'..msg.chat_id_) then
local get_me = function(extra, result)
local msgs = (tonumber(redis:get(DEVIL.."msgs:"..msg.sender_user_id_..":"..msg.chat_id_)) or "0" )
local kptext = (tonumber(redis:get(DEVIL.."text:"..msg.sender_user_id_..":"..msg.chat_id_.."")) or "0" )
local kpsticker = (tonumber(redis:get(DEVIL.."sticker:"..msg.sender_user_id_..":"..msg.chat_id_.."")) or "0" )
local pkPhoto = (tonumber(redis:get(DEVIL.."Photo:"..msg.sender_user_id_..":"..msg.chat_id_.."")) or "0" )
local kpVoice = (tonumber(redis:get(DEVIL.."Voice:"..msg.sender_user_id_..":"..msg.chat_id_.."")) or "0" )
local kpGif = (tonumber(redis:get(DEVIL.."Gif:"..msg.sender_user_id_..":"..msg.chat_id_.."")) or "0" )
local kpVideo = (tonumber(redis:get(DEVIL.."Video:"..msg.sender_user_id_..":"..msg.chat_id_.."")) or "0" )
local kpSelfVideo = (tonumber(redis:get(DEVIL.."SelfVideo:"..msg.sender_user_id_..":"..msg.chat_id_.."")) or "0" )
local kpcon = (tonumber(redis:get(DEVIL.."kpaddcon"..msg.sender_user_id_..":"..msg.chat_id_.."")) or "0" )
if result.first_name_ then
if #result.first_name_ < 25 then
else
for kkkkk in string.gmatch(result.first_name_, "[^%s]+") do
...
--------------------------------------------------------
if text == ''..(redis:get(DEVIL..'DEVILnams') or 'DEVIL')..' هينه' or text == ''..(redis:get(DEVIL..'DEVILnams') or 'DEVIL')..' رزله' then
function reep22(extra, result, success)
if tonumber(result.sender_user_id_) == tonumber(DEVIL_SUDO) then
send(msg.chat_id_, msg.id_, 1, 'انجب لك هذا مطوري العشق 😌💋', 1, 'md')
return false  end
if tonumber(result.sender_user_id_) == tonumber(bot_id) then
send(msg.chat_id_, msg.id_, 1, 'لك مگدر اهين نفسي 😞😂', 1, 'md')
return false  end
local DEVIL = { "لك دايح ، احترم نفسك لا بال 👠😠","ها مصراع تراچي ، اگعد راحه تره روحي طالعه 😐🍃","ها ابن الحنينه، ليش متسكت وتنجب 🌚"}
send(msg.chat_id_, result.id_, 1,''..DEVIL[math.random(#DEVIL)]..'', 1, 'md')
end
...
--------------------------------------------------------------------------------------------
if text == ''..(redis:get(DEVIL..'DEVILnams') or 'DEVIL')..' بوسه' or text == ''..(redis:get(DEVIL..'DEVILnams') or 'DEVIL')..' مصه' then
function reep22(extra, result, success)
if tonumber(result.sender_user_id_) == tonumber(DEVIL_SUDO) then
...
return false  end
local DEVIL = { "اععع 🤢خده بي حب شباب الوصخ😹😹","موااح 💋 مواااح  حياتي💋😌🍃","💋😞نسخ لصق ع الشفه 👄"}
send(msg.chat_id_, result.id_, 1,''..DEVIL[math.random(#DEVIL)]..'', 1, 'md')
end
if tonumber(msg.reply_to_message_id_) == 0 then
else
...
--------------------------------- rdood bot --------------------------------------------------
if not redis:get(DEVIL.."lock_reeeep"..msg.chat_id_) then
if text == "السلام عليكم" or text == "سلام عليكم" or text == "سلام" then
if not redis:get(DEVIL..'lock:add'..msg.chat_id_) then
local DEVIL = {"وعليكم السلام والرحمه⇣😻","يمه هلا بالغالي 😻🍃","وعليكم السلام حبيبي ☺️🍃","كافي بس تسلمون 🌝💔" }
send(msg.chat_id_, msg.id_, 1,''..DEVIL[math.random(#DEVIL)]..'', 1, 'md')
end
end
-----------------------------------------
...
if not redis:get(DEVIL..'lock:add'..msg.chat_id_) then
local DEVIL = {'اسمي '..(redis:get(DEVIL..'DEVILnams') or 'DEVIL')..' 😌👌','تره اسمي '..(redis:get(DEVIL..'DEVILnams') or 'DEVIL')..' يالجريذي ☹️😹'  }
send(msg.chat_id_, msg.id_, 1,''..DEVIL[math.random(#DEVIL)]..'', 1, 'md')
end
end
-----------------------------------------
if text == ''..(redis:get(DEVIL..'DEVILnams') or 'DEVIL')..'' then
if not redis:get(DEVIL..'lock:add'..msg.chat_id_) then
local DEVIL = {'اي نعم اني '..(redis:get(DEVIL..'DEVILnams') or 'DEVIL')..' 🍃😐','اهو اجوي الملطلطين 😹😪','كافي تره كرهت اسمي 💔😠'}
send(msg.chat_id_, msg.id_, 1,''..DEVIL[math.random(#DEVIL)]..'', 1, 'md')
end
end
-------------------------------------------------
...
if not redis:get(DEVIL..'lock:add'..msg.chat_id_) then
local DEVIL = {"هلووووات  ⁽🙆♂✨₎ֆ","يمه هلا بالعافيه 😻🍃","لا هلا ولا مرحبه شلونك مشتاقين 😻😂","اخلاً وصخلا 😌😂" }
send(msg.chat_id_, msg.id_, 1,''..DEVIL[math.random(#DEVIL)]..'', 1, 'md')
end
end
-----------------------------------------
...
if not redis:get(DEVIL..'lock:add'..msg.chat_id_) then
local DEVIL = {"تمام وانت/ي 😘🍃","شعليك انت 🧐😂","بخير انت/ي شلونك/ج ☺️","تمام وانت/ي ‏ ᵛ͢ᵎᵖ💛﴾" }
send(msg.chat_id_, msg.id_, 1,''..DEVIL[math.random(#DEVIL)]..'', 1, 'md')
end
end
-----------------------------------------
...
if not redis:get(DEVIL..'lock:add'..msg.chat_id_) then
local DEVIL = {"عـٰٰـٰود لوتُٰي 🙀 يـٰگول بُِوت عبالـٰه طافٰـُٰي💔 ويضِٰل يمٰـٓسلت وينشٰٰر روابـٰٓط 😳🍃","اي بوت شتريد 😤" }
send(msg.chat_id_, msg.id_, 1,''..DEVIL[math.random(#DEVIL)]..'', 1, 'md')
end
end
-----------------------------------------
...
if not redis:get(DEVIL..'lock:add'..msg.chat_id_) then
local DEVIL = {"شكلولك عليه كاولي 😶😂","صوتي محلو للاسف 😌💔","اشعجـب كاطع بيه ياراحتي النفسيه 😂💔","حرام  الغنا 😐🍃" }
send(msg.chat_id_, msg.id_, 1,''..DEVIL[math.random(#DEVIL)]..'', 1, 'md')
end
end
-----------------------------------------
...
if not redis:get(DEVIL..'lock:add'..msg.chat_id_) then
local DEVIL = {"بايات 💛🍃","گلعه 😶💔","الله الله الله وياك 😻😂","ثيمالا 🌝✋🏾" }
send(msg.chat_id_, msg.id_, 1,''..DEVIL[math.random(#DEVIL)]..'', 1, 'md')
end
end
-----------------------------------------
...
if not redis:get(DEVIL..'lock:add'..msg.chat_id_) then
local DEVIL = {"منو ال أجوو👀😹","جااااوات  ₎✿💥😈 ❥" }
send(msg.chat_id_, msg.id_, 1,''..DEVIL[math.random(#DEVIL)]..'', 1, 'md')
end
end
-----------------------------------------
...
if not redis:get(DEVIL..'lock:add'..msg.chat_id_) then
local DEVIL = {"نورك/ج هذا ورده 🌝🍃","بوجودك/غلا تسلم 😻✨","انت/ي اصل النور 😋🍃","عماني نورك 😣😂" }
send(msg.chat_id_, msg.id_, 1,''..DEVIL[math.random(#DEVIL)]..'', 1, 'md')
end
end
-----------------------------------------
if text == "بوسني" or text == "بوسه" then
if not redis:get(DEVIL..'lock:add'..msg.chat_id_) then local DEVIL = {"مووووووووواححح💋😻","مابوس ولي😌😹","خدك/ج نضيف 😂🍃","البوسه بالف حمبي 🌝💋" }
send(msg.chat_id_, msg.id_, 1,''..DEVIL[math.random(#DEVIL)]..'', 1, 'md')
end
end
-----------------------------------------
...
if not redis:get(DEVIL..'lock:add'..msg.chat_id_) then
local DEVIL = {"عيـــب 🙀😹","شبيه امك حمبي😋🍃" }
send(msg.chat_id_, msg.id_, 1,''..DEVIL[math.random(#DEVIL)]..'', 1, 'md')
end
end
-----------------------------------------
...
if not redis:get(DEVIL..'lock:add'..msg.chat_id_) then
local DEVIL = {"شبيه الشكره ام الوصخ 🤭😹","حبيته فدوووه😻","شرايد من خالته 🤭😂","خالته تفلش 😶😹" }
send(msg.chat_id_, msg.id_, 1,''..DEVIL[math.random(#DEVIL)]..'', 1, 'md')
end
end
-----------------------------------------
...
if not redis:get(DEVIL..'lock:add'..msg.chat_id_) then
local DEVIL = {"تف عليك ادبسزز 😒😹","لا تتفل على وجهك 😻😹","ما اسمحلك هيلگ 😡😹","بدون تفال رجائاً 😹😹" }
send(msg.chat_id_, msg.id_, 1,''..DEVIL[math.random(#DEVIL)]..'', 1, 'md')
end
end
-----------------------------------------
...
if not redis:get(DEVIL..'lock:add'..msg.chat_id_) then
local DEVIL = {"كافـي ضحــك 😐","لتضحك هواي وتصير فاول 🌝😹","هذ شبي يضحك 🙀😳","اضحك هيه الدنيا خربانه 😂😂" }
send(msg.chat_id_, msg.id_, 1,''..DEVIL[math.random(#DEVIL)]..'', 1, 'md')
end
end
-----------------------------------------
...
if not redis:get(DEVIL..'lock:add'..msg.chat_id_) then
local DEVIL = {"لصير عصبي يرتفع ضغطـك 😌😂","صار وجه احمر مثل الطماطه 🙊😹","اوف شحلاتك وانت ضايج 😻","شبي هذا الله يستر 😼😹" }
send(msg.chat_id_, msg.id_, 1,''..DEVIL[math.random(#DEVIL)]..'', 1, 'md')
end
end
-----------------------------------------
if text == "😒" or text == "😒😒" or text == "😒😒😒" or text == "😒😒😒😒" then
if not redis:get(DEVIL..'lock:add'..msg.chat_id_) then
local DEVIL = {"لضوج متسوووه 🤭😹","يمه زعلان الحلو ما يكلي مرحبا 😻😹","اعدل وجهك لا اعدله الك/ج 😼👊🏼" }
send(msg.chat_id_, msg.id_, 1,''..DEVIL[math.random(#DEVIL)]..'', 1, 'md')
end
end
-----------------------------------------
...
if not redis:get(DEVIL..'lock:add'..msg.chat_id_) then
local DEVIL = {"يمه الگمر عذبني حبه 🙊😻","عو نضيف الوصخ 😹😹","طفي ضواك عميتني 😼😹" }
send(msg.chat_id_, msg.id_, 1,''..DEVIL[math.random(#DEVIL)]..'', 1, 'md')
end
end
-----------------------------------------
if text == "💋" or text == "💋💋" then
if not redis:get(DEVIL..'lock:add'..msg.chat_id_) then
local DEVIL = {"كبر 🙀 جان استحيتو 😹😹","عســـل 😋🙊" }
send(msg.chat_id_, msg.id_, 1,''..DEVIL[math.random(#DEVIL)]..'', 1, 'md')
end
end
-----------------------------------------
if text == "💔" or text == "💔💔" then
if not redis:get(DEVIL..'lock:add'..msg.chat_id_) then
local DEVIL = {"شبي مكسور 😔💔","موجوع كلبي والتعب بيه 😔😹","اكل بصل وانسه الحصل 😻😹" }
send(msg.chat_id_, msg.id_, 1,''..DEVIL[math.random(#DEVIL)]..'', 1, 'md')
end
end
-----------------------------------------
if text == "😻" or text == "😻😻" or text == "😍😍" or text == "😍" then
if not redis:get(DEVIL..'lock:add'..msg.chat_id_) then
local DEVIL = {"شوفو الحب صاعد فول 😻😹","ها ناوي تزحف 😹😹","فدوووه لهاي العيون 🙊😻" }
send(msg.chat_id_, msg.id_, 1,''..DEVIL[math.random(#DEVIL)]..'', 1, 'md')
end
end
-----------------------------------------
if text == "😐" or text == "🙂" then
if not redis:get(DEVIL..'lock:add'..msg.chat_id_) then
local DEVIL = {"شبيك حبيبي 😂💔","منور محمد الاعمى 😐😹" }
send(msg.chat_id_, msg.id_, 1,''..DEVIL[math.random(#DEVIL)]..'', 1, 'md')
end
end
-----------------------------------------
...
if not redis:get(DEVIL..'lock:add'..msg.chat_id_) then
local DEVIL = {"دووم الضحكه 🙊🍃","دوم الضحكه ℡̮⇣┆👑😻⇣ۦ ٰ" }
send(msg.chat_id_, msg.id_, 1,''..DEVIL[math.random(#DEVIL)]..'', 1, 'md')
end
end
-----------------------------------------
...
if not redis:get(DEVIL..'lock:add'..msg.chat_id_) then
local DEVIL = {"اي غرد/ي دا اسمع 👂🏽😹","كول😹 (كول لو هدف)😔😹","ها حياتي 🙊" }
send(msg.chat_id_, msg.id_, 1,''..DEVIL[math.random(#DEVIL)]..'', 1, 'md')
end
end
-----------------------------------------
...
if not redis:get(DEVIL..'lock:add'..msg.chat_id_) then
local DEVIL = {"انت/ي شبيك/ج😣","مابيه شي تسلم 💋😹" }
send(msg.chat_id_, msg.id_, 1,''..DEVIL[math.random(#DEVIL)]..'', 1, 'md')
end
end
----------------------------------------
if text == "🌚💔" or text == "💔" then
if not redis:get(DEVIL..'lock:add'..msg.chat_id_) then
local DEVIL = {"هم اجانه محترك وجه😂♥️","هاي منو كاسر كلبك😡","اهو هم اجانه صخام🐸👌" }
send(msg.chat_id_, msg.id_, 1,''..DEVIL[math.random(#DEVIL)]..'', 1, 'md')
end
end
---------------------------------------
...
if not redis:get(DEVIL..'lock:add'..msg.chat_id_) then
local DEVIL = {"ها بدت حبجيه ✨😂","لتلح عود يعني احبج🙈😹","كافي درينه مشتاقله 😒" }
send(msg.chat_id_, msg.id_, 1,''..DEVIL[math.random(#DEVIL)]..'', 1, 'md')
end
end
---------------------------------------
if text == "😢" or text == "😢😢" then
if not redis:get(DEVIL..'lock:add'..msg.chat_id_) then
local DEVIL = {"يبجي دلوع😜😹","هاي عود انت جبير كاعد تبجي😑💔","لتلح درينه تبجي😒"}
send(msg.chat_id_, msg.id_, 1,''..DEVIL[math.random(#DEVIL)]..'', 1, 'md')
end
end
--------------------------------------
...
if not redis:get(DEVIL..'lock:add'..msg.chat_id_) then
local DEVIL = {"علساس انت متفاعل😒😒","اي عندك اعتراض🤔","اني معليه احرسكم😎" }
send(msg.chat_id_, msg.id_, 1,''..DEVIL[math.random(#DEVIL)]..'', 1, 'md')
end
end
------------------------------------
...
if not redis:get(DEVIL..'lock:add'..msg.chat_id_) then
local DEVIL = {"عود اني حباب ونت شيطان يتعلم منك🙈😂😂","وجهك ميساعد🤢😂" }
send(msg.chat_id_, msg.id_, 1,''..DEVIL[math.random(#DEVIL)]..'', 1, 'md')
end
end
-----------------------------------------
...
if not redis:get(DEVIL..'lock:add'..msg.chat_id_) then
local DEVIL = {"اهو لا هلا بيه ✨😂","خي ولي مزاعله ✨😂","اهلا بيه بس اطرده اذا اجه😒😂" }
send(msg.chat_id_, msg.id_, 1,''..DEVIL[math.random(#DEVIL)]..'', 1, 'md')
end
end
------------------------------------------------------------
...
if not redis:get(DEVIL..'lock:add'..msg.chat_id_) then
local DEVIL = {"وخرو وخرو🤓 هلا بلخميس تيرارا وياي يلا😍😹","هلا بلخميس عطله وكذا ركصو يلا😍😂" }
send(msg.chat_id_, msg.id_, 1,''..DEVIL[math.random(#DEVIL)]..'', 1, 'md')
end
end
-----------------------------------------
if text == "🙊" or text == "🙈" then
if not redis:get(DEVIL..'lock:add'..msg.chat_id_) then
local DEVIL = {"اول مره اشوف قرد يستحي🤔😂","ما مرجيه منك هايه صاير تستحي انته هوايه 😍😂😂" }
send(msg.chat_id_, msg.id_, 1,''..DEVIL[math.random(#DEVIL)]..'', 1, 'md')
end
end
--------------------------------------
...
if not redis:get(DEVIL..'lock:add'..msg.chat_id_) then
local DEVIL = {"ها ها يمعودين احنه هنا😒😹","اعتقد اكو خاص وخطبو وهنا زفه بسيارتي🚗😂" }
send(msg.chat_id_, msg.id_, 1,''..DEVIL[math.random(#DEVIL)]..'', 1, 'md')
end
end
end
...
if idf:match("-100(%d+)") and not redis:get(DEVIL.."bot:muted:Time" .. chat .. ":" .. user) and redis:sismember(DEVIL.."bot:muted:" .. chat, user) then
redis:srem(DEVIL.."bot:muted:" .. chat, user)
end
...
if (text:match("[Tt][Ee][Ll][Ee][Gg][Rr][Aa][Mm].[Mm][Ee]") or text:match("[Tt][Ll][Gg][Rr][Mm].[Mm][Ee]") or text:match("[Tt].[Mm][Ee]")) and redis:get(DEVIL.."bot:links:mute" .. result.chat_id_) then
local msgs = {
[0] = data.message_id_
}
delete_msg(msg.chat_id_, msgs)
end
if result.content_.entities_ and result.content_.entities_[0] and (result.content_.entities_[0].ID == "MessageEntityTextUrl" or result.content_.entities_[0].ID == "MessageEntityUrl") and redis:get(DEVIL.."bot:webpage:mute" .. result.chat_id_) then
local msgs = {
[0] = data.message_id_
}
delete_msg(msg.chat_id_, msgs)
end
if result.content_.web_page_ and redis:get(DEVIL.."bot:webpage:mute" .. result.chat_id_) then
local msgs = {
[0] = data.message_id_
}
delete_msg(msg.chat_id_, msgs)
end
if (text:match("[Hh][Tt][Tt][Pp]") or text:match("[Ww][Ww][Ww]") or text:match(".[Cc][Oo]") or text:match(".[Oo][Rr][Gg]") or text:match(".[Ii][Rr]")) and redis:get(DEVIL.."bot:webpage:mute" .. result.chat_id_) then
local msgs = {
[0] = data.message_id_
}
delete_msg(msg.chat_id_, msgs)
end
if text:match("@") and redis:get(DEVIL.."tags:lock" .. msg.chat_id_) then
local msgs = {
[0] = data.message_id_
}
delete_msg(msg.chat_id_, msgs)
end
if text:match("#") and redis:get(DEVIL.."bot:hashtag:mute" .. result.chat_id_) then
local msgs = {
[0] = data.message_id_
}
delete_msg(msg.chat_id_, msgs)
end
if text:match("[\216-\219][\128-\191]") and redis:get(DEVIL.."bot:arabic:mute" .. result.chat_id_) then
local msgs = {
[0] = data.message_id_
}
delete_msg(msg.chat_id_, msgs)
...
if redis:get(DEVIL..'bot:english:mute'..result.chat_id_) then
local msgs = {[0] = data.message_id_}
delete_msg(msg.chat_id_,msgs)
end
end
if redis:get(DEVIL..'editmsg'..msg.chat_id_) == 'delmsg' then
local id = msg.message_id_
local msgs = {[0] = id}
local chat = msg.chat_id_
delete_msg(chat,msgs)
elseif redis:get(DEVIL..'editmsg'..msg.chat_id_) == 'didam' then
if redis:get(DEVIL..'bot:editid'..msg.message_id_) then
local old_text = redis:get(DEVIL..'bot:editid'..msg.message_id_)
...
redis:sadd(DEVIL.."groups:users" .. msg.chat_id_, msg.sender_user_id_)
if text then
...
redis:set(DEVIL.."Bot:KpBotAccount", data.value_.value_)
end end end end  end
----- End Source By Karrar DEVIL »»»»»»»»»»»»»»»»»»»
